...
`plans/rationale/fastapi.md` for why this is safe from outside FastAPI's own code.

Requires `fastapi` and `httpx`, which `voci/__init__.py` does not import. Use
`import voci.fastapi`.
"""
...
from voci._di.fixtures import Fixture, fixture
...
class _Layers[K]:
    ...
    app's own values. One instance per proxy, so per app object: voci never puts two apps'
...
class _LayeredOverrides(MutableMapping[Override, Override]):
    ...
    def __init__(self, base: MutableMapping[Override, Override]) -> None:
        self._base = base
        self._layers: _Layers[Override] = _Layers("voci.fastapi.dependency_overrides")
    ...
    def clear(self) -> None:
        ...
        ends on. Under voci it is a no-op you can delete — the layer is dropped when `client()`
    ...
    def __repr__(self) -> str:
        ...
        return f"<voci dependency_overrides: {mine} in this test, {len(self._base)} on the app>"
...
class _LayeredState(State):
    ...
    def __init__(self, base: dict[str, Any]) -> None:
        ...
        object.__setattr__(self, "_layers", _Layers("voci.fastapi.state"))
    ...
    def __getattr__(self, key: Any) -> Any:
        ...
        # without risking another call into `__getattr__` — see plans/rationale/fastapi.md
        # ("the view of it becomes per-test") for the fuller shape of this.
        try:
...
class _Install:
    """What voci has swapped in on one app, and what was there before.
...
_REPLACED = """\
voci.fastapi.client(): app.{attribute} was replaced after voci installed its per-test layer.
...
every concurrently-running test was reading through, so voci is stopping rather than running
them against the wrong dependencies.

Delete the reset. Under voci each `client()` context drops its own layer on exit and touches
nothing else, so there is nothing left to clean up.\
"""

_DELETE_OUTSIDE_LAYER = """\
voci.fastapi: cannot `del app.dependency_overrides[{key}]` from inside a test.
...
def _install(app: FastAPI) -> _Install:
    ...
    voci is single-threaded async and nothing below suspends, so the check and the set cannot
    interleave with another test's — a lock here would be a claim about threads that voci does
    ...
    says so explicitly. Fine inside a voci run; anything elsewhere in the same process that goes
    on serving `app` after import (a notebook, an embedded uvicorn) keeps voci's proxy too, which
...
async def client(
    ...
    *,
    ...
    :param overrides: `{dependency: replacement}`, with FastAPI's exact semantics. The value is a
        *dependency callable*, so a fixture value is passed as `lambda: session`. voci does not
