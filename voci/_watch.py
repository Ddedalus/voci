...
(`voci._collection.index`) is what makes a rerun cheap when nothing collection-relevant changed,
a poll loop over `discover_files`'s own walk (below) is the watcher, and `--lf` (`voci._collection
...
test file is picked up the same way restarting `voci` by hand would pick it up; only the set of
...
#: broken `[tool.voci]` table is exactly the kind of thing a following edit fixes.
_ABORTED = 2

_WATCHING_MESSAGE = "voci: watching for changes (Ctrl-C to stop)"
