...
current_test_context: ContextVar[TestContext | None] = ContextVar(
    "voci_current_test_context", default=None
)
...
_SESSION_DIR_RE = re.compile(r"^voci-(\d+)$")
...
def _basetemp_default_root() -> Path:
    return Path(tempfile.gettempdir()) / f"voci-of-{_current_user()}"


#: Dropped into every basetemp root this module creates, and checked before ever
#: rmtree-ing one -- makes the destructive path opt-in to directories voci itself
...
BASETEMP_MARKER_NAME = ".voci-basetemp"
...
#: another voci process is still writing into, since every voci on a machine shares
#: one parent directory and sweeps it on startup.
SESSION_LOCK_NAME = ".voci-lock"
...
#: `voci-<n>` namespace the sweep scans.
...
def _is_in_use(root: Path) -> bool:
    """Whether a voci run may still be writing into `root`. Age is the fallback, not a
...
def _discard_session_root(root: Path) -> None:
    """Delete `root`, renaming it out of the `voci-<n>` namespace first so that a
...
def _allocate_session_root(parent: Path, *, retention: int) -> tuple[Path, Callable[[], None]]:
    """One fresh, numbered `voci-<n>` directory under `parent`, with the callable that
    releases this process's claim on it, and the retention policy applied: the most
    recent `retention` previous roots are kept, and so is any root another voci run
    still holds.
    ...
    while True:
        root = parent / f"voci-{next_n}"
        try:
            root.mkdir()
        except FileExistsError:
            # Another voci took this number in the moment since `parent.iterdir()`.
            next_n += 1
        else:
            break
    ...
    if explicit is not None:
        ...
        if root.exists():
            if not (root / BASETEMP_MARKER_NAME).is_file():
                raise ValueError(
                    f"--basetemp {root} already exists and does not look like a previous voci "
...
def install(
    *,
    ...
    if _installed is not None:
        ...
        if mismatched:
            raise RuntimeError(
                f"voci._builtins.capture.install() was already called with passthrough="
...
def _require_test_context() -> TestContext:
    ...
    `None` here means a voci internal bug, not a user mistake.
    """
    tc = current_test_context.get()
    if tc is None:
        raise RuntimeError(
            "a builtin capture fixture (capture/log_records/test_info/tmp_path) was constructed "
            "outside any test's envelope -- this is a voci internal error: "
...
def _require_installed() -> CaptureSetup:
    ...
    if _installed is None:
        raise RuntimeError(
            "voci._builtins.capture.install() has not been called -- tmp_path/tmp_path_factory "
...
async def capture_provider(
    ...
    """`voci.capture`'s `BuiltinProvider`. `ctx` is unused -- everything this needs
...
async def log_records_provider(
    ...
    """`voci.log_records`'s `BuiltinProvider`. Hands `LogRecords` the live deque
...
async def test_info_provider(
    ...
    """`voci.test_info`'s `BuiltinProvider`. `id`/`module_path` come from `ctx`;
...
async def tmp_path_provider(
    ...
    """`voci.tmp_path`'s `BuiltinProvider`: `basetemp/<sanitized-id>`, unique by
    construction."""
...
async def tmp_path_factory_provider(
    ...
    """`voci.tmp_path_factory`'s `BuiltinProvider`. Session-scoped:
...
async def tmpdir_provider(
    ...
    """`voci.tmpdir`'s `BuiltinProvider`: its own `Depends(tmp_path)` directory, wrapped in
    `LegacyPath`."""
    del ctx
...
async def tmpdir_factory_provider(
    ...
    """`voci.tmpdir_factory`'s `BuiltinProvider`: its own `Depends(tmp_path_factory)` factory,
