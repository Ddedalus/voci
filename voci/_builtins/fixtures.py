...
`voci._builtins.capture`, wired in at the bottom of this module.
...
from voci._di.fixtures import Depends, builtin_fixture, fixture
...
_RUNTIME = "provided by the voci runtime; not callable directly"
...
class TestInfo:
    ...
    `@voci.timeout(...)` mark if it carries one, else the suite's `--timeout`."""
    worker: int
...
class _CapturedText(Protocol):
    ...
    `voci._builtins.capture.Sink`, in practice."""
...
# Rewire the five fixtures above onto `voci._builtins.capture`'s providers, turning each from a
...
# rationale/builtins-fixtures.md ("import cycle is deliberate") for the full shape of it.
from voci._builtins import capture as _capture  # noqa: E402
