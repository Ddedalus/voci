...
from voci._collection.collect import Skipped, TestRecord
from voci._report import color as _color
from voci._run.run import FAILING_OUTCOMES, Outcome, TestResult, solo_for_patching
...
#: block's path column, an id is what the reader copies back into the next `voci` invocation.
_ID_COLUMN_WIDTH = 56
...
class Reporter:
    ...
    def _print_file_block(self, path: Path, results: list[TestResult]) -> None:
        ...
        `voci.Skipped` for every one of them. `STOP` is the status of a file the run was
        ...
        # A `voci.Skipped` raised mid-run, folded into the same annotation as a skip mark's:
        ...
        # are *all* `voci.Skipped` has nothing here, exactly like a file with no results at
        ...
        *,
        ...
        totals = [
            ...
            self._counts(
                ...
                # (`voci.Skipped`, mid-setup or mid-call) read as one count: both mean the
    ...
    def _print_skip_reasons(self, results: list[TestResult]) -> None:
        ...
        (kept `results` out entirely, listed from `self.skipped`) and a `voci.Skipped` raised
