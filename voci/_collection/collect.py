...
`func`, its `params` (if `@voci.parametrize`d), its resolved `plan`, and its `marks`, whose
conditions are decided here so nothing downstream evaluates a suite's own expression again. A test
marked `@voci.skip`/`@voci.skipif` is read off the function object and excluded from `records`
into `skipped` instead of running for real; a `tag_expr` (see `selection.py`) whose expression a
test's `@voci.tag(...)` names don't satisfy excludes it into `deselected` instead -- checked
...
`@voci.parametrize` case's own `[case]` suffix. A `Depends(...)`-defaulted parameter is resolved
via `_fixtures.plan_for`, alongside the fixtures the module declared for all of its tests with
`voci.use(...)` (`requires.py`), and a malformed DI graph (bad scope nesting, a missing
...
A `@voci.parametrize`d test expands into one record per case
(`parametrize.cases_for`), sharing the one `ResolutionPlan` built for the function — parametrize
values are call kwargs, not part of the DI graph. A case written as `voci.case(value, marks=...)`
...
Import mechanics: importlib only, path-derived module names under `voci_tests.*`, one entry per
...
do instead (`_shape_problem`, `_class_problems`): a class voci can't construct or whose
...
from voci._collection.selection import KeywordExpression, TagExpression
...
class TestRecord:
    ...
    """This case's `@voci.parametrize` values, keyed by argument name — passed to `func` as
    extra kwargs alongside `plan`'s injected ones. `None` for a test with no `parametrize` mark;
    an empty dict never occurs (`@voci.parametrize` rejects an empty `argnames`)."""
    plan: ResolutionPlan
    ...
    fixtures" branch. Identical (the same object) across every `@voci.parametrize` case of the
    ...
    marks: Marks
    """This record's marks: the function's own, with the marks of its `voci.case(...)` cases
    folded in, and every condition already decided -- a `@voci.xfail(condition=...)` that does
...
class CollectionError:
    ...
    (bad scope nesting, a missing injection), or a test shape voci refuses to collect
...
class CollectionResult:
    ...
    A test excluded before its `@voci.parametrize` cases are built has no per-case ids to be
    listed under, and `voci.skip` plus `tag_expr` both exclude that early -- deliberately, since
...
def module_name_for(path: Path, rootdir: Path) -> str:
    """`voci_tests.<dotted.relpath.without.suffix>`.
...
def _cases_carry_tags(marks: Marks) -> bool:
    """Whether any `voci.case(...)` in this test's parametrizations carries a tag.
...
def collect(  # noqa: C901
    ...
    *,
    ...
    1. Read what the packages above the file declared with `voci.use(...)`
    ...
    3. Within the imported module, find the tests *defined* in it (`_module_candidates`) — i.e.
       ...
       elsewhere isn't collected twice, nor given the importing module's `voci.use(...)`
       declarations. That is every module-level `test_*` function plus the `test_*` methods of
       every `class Test*`, which voci treats as pure namespacing. A shape that would collect
    ...
    5. Per test: a `skip`/truthy-`skipif` mark on the function excludes it from `records` into
       ...
       `@voci.tag(...)` names don't satisfy it into `deselected`, and a skip outranks it: a
       ...
       `voci.use(...)` fixtures reaching it introduce — or a name collision between stacked
       `@parametrize`s, excludes it into `errors` instead. Otherwise build one
       `TestRecord` per case in the cartesian product of `@voci.parametrize`'s cases and the
       ...
       `voci.case(..., marks=...)` case is skipped or deselected there too, on its own merged
    ...
    The `voci.use(...)` declarations reaching a file are validated once, before its tests: a
    malformed declared graph is one `CollectionError` for the file, which then contributes no
    records. Once every file is done, a `voci.use(...)` call left on a module that is neither a
    collected test file nor a package voci read declarations from becomes a `CollectionError` of
    ...
    #: down from -- rather than only the ones this call collects. A `voci.use(...)` in a test
...
def _misplaced_declarations(
    ...
    """`voci.use(...)` declarations on modules that are neither collected test files nor
    packages above one.

    A declaration is read back off the modules voci imported for itself, so a call in a shared
    helper module is never seen and the fixtures it names never run. Reported rather than left as
    a missing side effect. A module voci imported reaches this scan only when something else
    imported it under its real name too, hence the `declaring_files` exemption; voci's own
    ...
    for name, module in list(sys.modules.items()):
        ...
        misplaced.append(
            CollectionError(
                ...
                message=(
                    f"{name}: voci.use(...) applies to the tests in the container that calls it, "
                    f"and this module is neither a test module voci collects nor a package "
...
def _import_failure(error: Exception, path: Path) -> str:
    """The traceback for a failed import, plus what to do about the one failure mode voci's own
    import machinery causes.

    A file is imported by path under a synthetic `voci_tests.*` name with no real package behind
    it, so a relative import inside one resolves against nothing and fails naming `voci_tests` --
    ...
    if isinstance(error, ModuleNotFoundError) and (error.name or "").startswith(_MODULE_PREFIX):
        message += (
            f"\nvoci imports {path} by its path rather than through its package, so a relative "
...
def _no_dependencies() -> None:
    """Stand-in test body: a function whose plan is whatever `voci.use(...)` declared and
    nothing else."""
...
class _Candidate:
    ...
    func: Callable[..., object]
    """What voci calls -- for a method, the `_receiving` wrapper that constructs its receiver."""
...
def _module_candidates(module: object, module_name: str) -> tuple[list[_Candidate], list[str]]:
    ...
    for candidate in candidates:
        if _yields(real_function(candidate.func)):
            # A generator function's body doesn't run when it is called -- voci would get an
            # iterator back, never iterate it, and report a test that passed without executing
            # a single line of itself.
            problems.append(
                f"{candidate.name}: a test that yields never runs its own body -- voci calls it "
...
def _group_bases(objects: Iterable[object], module_name: str) -> frozenset[type]:
    ...
    `class TestPostgres(SharedTests)`) is not a group voci lost -- its tests run through each
...
def _class_candidates(
    ...
    module level), plus a message per shape voci refuses to collect silently.
    ...
    if problems:
        # Nested groups go with it: a class voci can't honor can't host them either.
        return [], problems
...
#: Per-test and per-class lifecycle hooks voci has no equivalent of: a fixture is what runs
...
#: Class-name endings that mean "this is a suite of tests" to a reader, and so to voci when it
...
def _class_problems(cls: type, methods: list[tuple[str, Callable[..., object], int]]) -> list[str]:
    ...
    if "__init__" in own:
        problems.append(
            f"{cls.__qualname__}: voci constructs a test class with no arguments, once per test, "
            f"and this one defines __init__. Move what it sets up into a fixture {names} depend "
            f"on, or rename the class so voci doesn't collect it."
        )
    ...
    if marks_of(cls) != Marks():
        problems.append(
            ...
            f"@voci.skip/@voci.tag/... to each of {names} instead."
        )
    return problems
...
def _misnamed_group_problem(
    ...
    return (
        f"{cls.__qualname__}: voci collects test methods from classes named `Test*` only, so "
...
def _test_methods(cls: type) -> list[tuple[str, Callable[..., object], int]]:
    """The `test_*` methods `cls` provides, inherited ones included: each one's name, what voci
...
def _receiving(func: Callable[..., Any], receiver: Callable[[], object]) -> Callable[..., object]:
    """`func` as voci calls it, with `receiver()` passed as its first argument per call.
...
def _shape_problem(name: str, obj: object, module_name: str) -> str | None:
    """The message for a module-level `test_*` name bound to a function voci doesn't collect, or
    `None`.
    ...
    if getattr(obj, "__module__", None) != module_name or obj.__name__.startswith("test_"):
        return None
    return (
        f"{name}: voci collects `def`/`async def` functions whose own name starts with `test_`, "
