"""`voci.use(...)`: fixtures a container declares on behalf of the tests inside it.

A test module that calls `voci.use(db_reset)` gives every test it defines an extra dependency on
...
from voci._di.fixtures import Fixture
...
REQUIRES_ATTR = "__voci_requires__"
...
def use(*fixtures: Fixture[Any]) -> None:
    ...
    for fixture in fixtures:
        if not isinstance(fixture, Fixture):
            raise TypeError(
                f"voci.use() takes fixtures, got {type(fixture).__name__} -- pass the function "
                f"decorated with @voci.fixture(), not a call to it."
            )
...
def requires_of(module: object) -> tuple[Fixture[Any], ...]:
    """The fixtures `module` declared with `voci.use(...)`, in declaration order. Never raises."""
...
def _declaring_namespace() -> dict[str, Any]:
    ...
    if frame.f_code.co_name != "<module>":
        raise TypeError(
            "voci.use() applies to every test in the container declaring it, so it belongs in a "
