"""Expanding `@voci.parametrize` into one callspec per case.
...
from voci._di.fixtures import case_value_id, dedupe_case_ids
from voci._marks import NO_MARKS, Marks, ParamSet, merged
...
class Case:
    """One expanded `@voci.parametrize` callspec: its call kwargs, display id and own marks."""

    params: dict[str, object]
    id: str
    marks: Marks = NO_MARKS
    """What `voci.case(..., marks=...)` put on this case alone, folded across stacked
        ...
        for name in param_set.argnames:
            if name in seen:
                raise ValueError(
                    ...
                    f"@voci.parametrize on the same test"
                )
            seen.add(name)
...
def cases_for(parametrizations: tuple[ParamSet, ...]) -> tuple[Case, ...]:
    ...
    with another after generation. Marks a `voci.case(...)` wrapper put on any case of the
