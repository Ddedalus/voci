...
`__voci_marks__`. Every decorator returns the *same* function object with that attribute
...
MARKS_ATTR = "__voci_marks__"

type ExcTypes = type[BaseException] | tuple[type[BaseException], ...]

type MarkDecorator = Callable[[Any], Any]
"""What `case(marks=...)` takes: `voci.skip("...")`, `voci.solo`, or any other decorator that
...
# Marks live on the function object (`__voci_marks__` is part of the tested surface — see
...
def _amend[F: Callable[..., Any]](fn: F, **changes: Any) -> F:
    ...
    for field, value in changes.items():
        if field in _SCALAR_MARKS and getattr(current, field) is not None:
            ...
            raise TypeError(
                f"@voci.{field} applied twice to {name!r}: already set to "
...
def case(*values: object, marks: MarkDecorator | Sequence[MarkDecorator] = ()) -> ParamCase:
    ...
    `marks` are the very decorators a test carries -- `case(2, marks=voci.skip("flaky"))`,
    `case(3, marks=[voci.xfail("known"), voci.tag("slow")])` -- and mean for this case what
    ...
    marked.__name__ = "voci.case(...)"
    for decorator in decorators:
        if decorator(marked) is not marked:
            raise TypeError(f"case(marks=...): {decorator!r} is not a voci mark decorator")
    ...
    if decorators and collected == NO_MARKS:
        ...
        raise TypeError(f"case(marks=...): {marks!r} attached no voci marks")
    if collected.parametrizations:
        raise TypeError("case(marks=...): @voci.parametrize applies to a whole test, not a case")
