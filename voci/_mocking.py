...
from voci._builtins.capture import current_test_context
...
class Patching:
    ...
    `mock.patch.multiple` does. Supplied at call time exactly the way `@voci.parametrize`'s
...
def real_function(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    injection plan and its definition line off; the wrapper is still what voci calls. Only a
...
class GlobalPatchError(RuntimeError):
    """A `unittest.mock` patch entered from a test voci is running alongside others.
...
def _check(patcher: Any) -> None:
    ...
    voci scheduled to run by itself."""
    ...
    raise GlobalPatchError(
        ...
        f"test running at the same time sees it. Mark this test @voci.solo to run it alone, "
        f"or @voci.isolated to run it in its own subprocess. A patch applied as a decorator "
