...
This module owns the objects the rest of voci is built on: `Fixture`, what `@voci.fixture()`
...
cases, mirroring `@voci.parametrize`'s own case expansion for the fixture graph instead of a
    ...
    for index, param in enumerate(params):
        ...
        if from_default is not None and from_annotation is not None:
            raise DIError(
                ...
                f"in Annotated[...] metadata. Pick one spelling -- voci merges nothing, even "
        ...
        if index < code.co_posonlyargcount:
            raise DIError(
                ...
                f"'/') is not supported -- voci binds every injection by keyword, and a "
...
def _annotated_injections(
    ...
    exists only under `TYPE_CHECKING`. The policy both force is **parse, don't evaluate**: voci
    reads the annotation's source text with `ast` and evaluates only the metadata elements that
    are calls to voci's own `Depends`. The type half is never touched, so wherever the module
...
def _annotations_of(func: Callable[..., Any]) -> Mapping[str, Any]:
    ...
    if sys.version_info >= (3, 14):
        ...
        except Exception:  # a hand-written `__annotate__`; not voci's to repair
            return {}
...
def _markers_in_source(
    ...
    and evaluated iff its callee is voci's own `Depends`. The gate is the shape of the
...
def _marker_of(
    ...
    # Anything that is not a call to voci's own `Depends` is left entirely alone: metadata holds
    # arbitrary objects, and voci must not execute arbitrary annotation code to look at them.
    ...
    try:
        ...
        value = eval(compile(ast.Expression(body=element), "<voci annotation>", "eval"), globalns)
...
def _is_annotated(node: ast.expr, globalns: dict[str, Any]) -> bool:
    ...
    # strength of it, since a marker still has to resolve to voci's own `Depends` by identity.
...
def _lookup_parts(parts: Sequence[str], globalns: dict[str, Any]) -> Any:
    ...
    A name that resolves to nothing is not an error: most annotations are types voci has no
...
#: The name a parametrized fixture's own function must accept its case value through — voci has
#: no `request` object, so `params=` reuses the same name-based convention `@voci.parametrize`
...
def case_value_id(
    ...
    `f"{argname}{index}"`. Shared by `@voci.parametrize`'s own id generation
...
def dedupe_case_ids(ids: Sequence[str]) -> tuple[str, ...]:
    ...
    already claimed by an earlier rename. Shared by `@voci.parametrize`'s own case expansion
    (`_collection.parametrize`) and by a parametrized fixture's `case_ids`.
    """
    ids = tuple(ids)
    counts = Counter(ids)
...
def _normalize_params(
    ...
    if not values:
        ...
        # no count discrepancy visible at a glance -- the same failure mode `@voci.parametrize`
...
class Fixture[T]:
    ...
    Constructed by `@voci.fixture()`. Immutable.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # `known_params` mechanism `@voci.parametrize` uses on a test, so a parametrized
    ...
    def provider(self) -> BuiltinProvider | None:
        """Set on voci's own built-in fixtures, whose value the runtime supplies directly
        rather than by calling `func`. `None` for every fixture built with `voci.fixture()`."""
...
class FixtureDecorator(Protocol):
    """What `voci.fixture(...)` returns: a decorator that unwraps the yielded/awaited type."""
...
def fixture(
    *,
    ...
    :param params: case values that multiply every test transitively depending on this fixture,
        ...
        parameter literally named `param` -- voci has no `request` object, so this is the same
        name-based convention `@voci.parametrize` uses for its own call kwargs.
...
def builtin_fixture[T](declared: Fixture[T], /, *, provider: BuiltinProvider) -> Fixture[T]:
    """Rebuild `declared` as a fixture whose value the voci runtime supplies directly, instead of
...
class ResolutionPlan:
    ...
    step nothing here points at came from a `voci.use(...)` declaration: built, cached and torn
    ...
    directly — through a `Depends(...)` site or a `voci.use(...)` declaration — i.e. the
...
def plan_for(
    ...
    *,
    ...
    `known_params` names parameters `@voci.parametrize`'s argnames supply at call time, so the
    ...
    `implicit` names fixtures `func`'s containers declared with `voci.use(...)`, in the order they
    ...
    in itself at call time -- `@mock.patch`'s mock objects, the one case voci recognizes
    ...
    # `params=` fixture reached only through `voci.use(...)` still has to fan the test out into
...
def expand_cases(plan: ResolutionPlan) -> tuple[ExpandedPlan, ...]:
    ...
    cases (e.g. two `@voci.parametrize` cases of the same function) still sees the same object.
    """
...
def _check_missing_injections(
    ...
    *,
    ...
    _reject_params(
        ...
        "are both Depends(...)-injected and supplied externally (e.g. by @voci.parametrize) "
        "-- pick one source per parameter.",
    )

    positional = code.co_varnames[: code.co_argcount]
    kwonly = code.co_varnames[code.co_argcount : code.co_argcount + code.co_kwonlyargcount]
    # The parameters a decorator wrapping `func` binds positionally before voci's own keyword
    ...
    _reject_params(
        ...
        "are supplied externally (e.g. by @voci.parametrize) in a slot the decorator wrapping "
    ...
    _reject_params(
        ...
        "are supplied externally (e.g. by @voci.parametrize) on a positional-only parameter "
        "(before '/') -- voci always calls a test function by keyword. Move it after the '/'.",
    )
    ...
    if not (code.co_flags & inspect.CO_VARKEYWORDS):
        _reject_params(
            ...
            "are supplied externally (e.g. by @voci.parametrize) but match none of this "
    ...
    if missing:
        ...
        raise DIError(
            ...
            f"Depends(...) -- voci has no name-based fixture lookup, so an uninjected "
