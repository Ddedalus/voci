...
A blocking call inside an `async def` test -- or inside a fixture, which voci constructs on the
...
from voci._assertions._vendor.saferepr import saferepr
...
#: enough that an ordinary slow fixture (which voci does construct on the loop) doesn't trip
...
#: Where voci's own frames live. A reported stack is cut at the innermost of them: everything
#: outside is voci dispatching a test (and asyncio dispatching voci), everything inside is the
#: test's own call chain, which is the only part that answers "what is it blocked on".
_VOCI_DIR = f"{Path(__file__).resolve().parent.parent}{os.sep}"
...
def _thread_stack(ident: int) -> list[traceback.FrameSummary]:
    ...
    Falls back to the raw innermost frames when the cut leaves nothing -- voci blocking its own
    ...
    below_voci: list[tuple[Any, int]] = []
    for candidate, lineno in frames:
        if candidate.f_code.co_filename.startswith(_VOCI_DIR):
            break
        below_voci.append((candidate, lineno))
    stack = traceback.StackSummary.extract(reversed(below_voci or frames))
...
def _owning_test(ident: int, test_ids: Mapping[CodeType, str]) -> str | None:
    ...
    two tests share, session teardown), and a code object two records share (one `@voci
...
class LoopWatchdog:
    ...
    def start(self) -> None:
        ...
        self._schedule()
        self._thread = threading.Thread(target=self._watch, name="voci-loop-watchdog", daemon=True)
        self._thread.start()
    ...
    def _tick(self) -> None:
        ...
        if stalled_at is not None:
            ...
            self._report(f"voci: the event loop is running again after {now - stalled_at:.1f}s")
        self._beat = now
        self._schedule()
    ...
    def _describe(self, blocked: float) -> str:
        lines = [f"voci: the event loop has been blocked for {blocked:.1f}s"]
...
def stuck_calls() -> str | None:
    ...
    lines = [
        f"voci: {count} sync {'test is' if count == 1 else 'tests are'} still running in a "
...
_unawaited: ContextVar[list[Unawaited] | None] = ContextVar(
    "voci_unawaited_coroutines", default=None
)
    ...
    if returned is not None:
        ...
        if awaitable_name is not None:
            ...
            details.append(
                ...
                f"An `async def` function that is called but not awaited never runs; voci "
            ...
            details.append(
                ...
                f"voci never inspects what a test returns. If that value was the check, "
