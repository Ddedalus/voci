"""Coverage measurement carried across the `@voci.isolated` process boundary.
...
which voci does not depend on coverage.py to determine -- `Coverage.current()` answers it.

A measurement that can't be made is reported through the run's `note`, never raised or warned:
what voci failed to record says nothing about the test that just ran.
...
_TOO_OLD = (
    "voci: coverage.py is measuring this run but is too old to extend that measurement into "
    "@voci.isolated subprocesses (7.10 or newer is needed). Lines that only an isolated test "
...
def subprocess_env(data_file: Path, *, note: Callable[[str], None]) -> dict[str, str] | None:
    """Environment putting a `@voci.isolated` subprocess under this process's own measurement,
    ...
    # (rationale/coverage.md). Parallel mode because the subprocess is not necessarily the
...
def harvest(data_file: Path, *, note: Callable[[str], None]) -> None:
    ...
    for path in written:
        ...
        try:
            ...
            note(f"voci: could not merge coverage data from {path}: {exc}")
