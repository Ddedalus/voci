"""The run cache under `rootdir/.voci_cache`: which tests failed last time, and which files
failed to collect at all.

Written once at the end of every run that executed tests, read at the start of a run that asks
for `--lf`/`--ff` (`voci._collection.lastfailed`). Every filesystem failure here is swallowed:
...
#: so a project has one voci-owned directory to ignore rather than two.
CACHE_DIR_NAME = ".voci_cache"
...
_GITIGNORE = "# Created by voci automatically.\n*\n"
...
def save(rootdir: Path, last_run: LastRun) -> None:
    ...
    # A distinct name per process, replaced into place: two voci runs sharing a rootdir must
...
def ensure_gitignore(rootdir: Path) -> None:
    ...
    behind, and a project must pick up no diff for having run voci.
    """
    directory = rootdir / CACHE_DIR_NAME
    if directory.is_dir():
        with contextlib.suppress(OSError):
