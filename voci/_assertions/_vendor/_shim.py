...
VOCI_REWRITER_REVISION = 2

#: What `rewrite.py` interpolates into its pyc tag. Deliberately not voci's package version:
#: a voci release that does not touch the rewriter should not throw away everyone's cache.
version = f"voci-r{VOCI_REWRITER_REVISION}"
...
#: Defaults for every ini option the vendored code reads. voci has no ini parser here; the
#: real values arrive from voci's own config layer (spec/02) when it constructs a `Config`.
_INI_DEFAULTS: dict[str, object] = {
    # Which modules the rewriter matches. voci widens this at the call site (spec/07 Q16).
...
class Config:
    ...
    Verbosity is a single number here; pytest supports per-kind overrides, which voci does
...
class FixtureFunctionDefinition:
    ...
    fixture's repr in an explanation. voci's fixtures are a different type entirely, so this
...
class AssertionState:
    ...
    Upstream carries the pytest plugin hook here too; voci's hook lives in ContextVars
...
def _no_trace(msg: str) -> None:
    """Tracing is off by default; voci swaps in a real sink under `--debug` (spec/02)."""
...
class _Outcomes:
    ...
    def __repr__(self) -> str:
        return "<voci assertion shim: outcomes>"
