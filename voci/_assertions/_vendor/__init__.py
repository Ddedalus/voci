"""pytest's assertion rewriter and explanation engine, vendored, not depended on: voci ships no
...
module globals), all of which voci replaces with `voci/_assertions/rewrite.py` and
`voci/_assertions/state.py`.

Import the pieces directly (`from voci._assertions._vendor import rewrite, util`); nothing is
