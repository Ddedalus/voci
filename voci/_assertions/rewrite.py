...
The rewriter itself is vendored in `voci/_assertions/_vendor/`; this module is the glue that turns
...
from voci._assertions._vendor import rewrite as _rewrite
from voci._assertions._vendor import truncate as _truncate
from voci._assertions._vendor import util as _util
from voci._assertions._vendor._shim import AssertionState, Config, Stash, StashKey
from voci._assertions._vendor._typing import NO_TRUNCATION_BUDGET, TruncationBudget
from voci._assertions.state import assertion_state
...
def resolve_cache_dir(
    ...
    """Where rewritten pycs go: an explicit path, then `VOCI_REWRITE_CACHE`, then
    `rootdir/.voci_cache/rewrite`, then (only when no `rootdir` is given) the
    platform's cache directory, then the interpreter's own pycache prefix, then a
    `.voci_cache` under `cwd()`.
    ...
    if rootdir is not None:
        return rootdir / ".voci_cache" / "rewrite"

    platform_cache = _platform_cache_dir()
    if platform_cache is not None:
        return platform_cache / "voci" / "rewrite"

    # Last resort: no home directory to put a cache in (containers often have none), so
    # borrow wherever the interpreter was already told to put pycs, in a voci-owned subdir.
    if sys.pycache_prefix:
        return Path(sys.pycache_prefix) / "voci-rewrite"

    return Path.cwd() / ".voci_cache" / "rewrite"
...
def _probe_writable(cache_dir: Path) -> str | None:
    ...
    # call, it was created solely to run the probe, and voci falls back to `plain` and never
...
def _discover_python_files(roots: Iterable[Path]) -> frozenset[Path]:
    ...
    also feeds the rewriter's name-based early-bailout set — pruning here is what keeps voci from
...
def plan(
    ...
    *,
    ...
    it's ignored once `cache_dir` or `VOCI_REWRITE_CACHE` already says where to put it.
    """
    ...
    if warn:
        ...
        print(
            f"voci: assertion rewriting disabled — {problem}\n"
            f"voci: falling back to --assert=plain (PEP 657 caret spans only). "
...
def uninstall(hook: object | None = None) -> None:
    """Remove voci's rewriting hook(s) from `sys.meta_path`. Idempotent."""
    global _installed_setup
...
def compare_explanation(op: str, left: object, right: object, config: Config | None) -> str | None:
    """voci's `_reprcompare`: what the rewriter splices into a failed comparison's message."""
