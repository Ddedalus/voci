...
- Vendored: 2026-09-08
- Destination: `voci/_assertions/_vendor/`
...
replaced by
`voci/_assertions/_vendor/_shim.py` (~130 LOC, hand-written).
...
- **TYPE_CHECKING: AssertionState from the shim** (x1, 2 upstream lines) — voci does not vendor pytest's assertion plugin glue
- **injected helper-module name** (x1, 1 upstream lines) — upstream bakes '_pytest.assertion.rewrite' into every generated pyc; a voci pyc that imports pytest's rewriter at exec time would be both wrong and a hidden dep
- **pyc tag carries a voci rewriter revision** (x1, 2 upstream lines) — so a change to voci's vendored codegen invalidates stale pycs, not just a CPython magic-number bump
...
- **temp pyc keyed on (pid, thread)** (x1, 1 upstream lines) — voci runs tests concurrently in one process; two threads importing different test modules would otherwise race on the same temp file name (spec/07 §4.2)
...
- **get_cache_dir honours voci's resolved cache root** (x1, 1 upstream lines) — voci resolves and probes one cache root at startup (spec/07 §5) rather than scattering pycs into every source tree
- **_warn_already_imported without pytest's config-time warning plumbing** (x1, 8 upstream lines) — issue_config_time_warning is pytest plugin machinery voci does not have
- **always-true-assert warning import** (x1, 4 upstream lines) — same
- **always-true-assert warning class** (x1, 3 upstream lines) — same
- **voci support block (cache root, pyc tail, warning type)** (x1, 1 upstream lines) — fresh code, appended near the top so the rest of the file can reference it
...
- **voci naming in the repr-failure message** (x1, 1 upstream lines) — the string is user-visible; it should not say 'pytest_assertion plugin'

### `_compare_any.py`

- **voci's Approx, with an optional _repr_compare** (x1, 7 upstream lines) — voci ships its own approx (spec/07 §8); the MVP one is scalars-only and has no detailed diff to offer, so the summary line has to stand on its own
- **_voci_approx_compare helper** (x1, 1 upstream lines) — fresh, so the branch above stays readable
