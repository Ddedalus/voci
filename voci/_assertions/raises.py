...
class RaisesContext[E: BaseException]:
    ...
    def __init__(
        ...
        # cancellation voci's own timeout machinery uses to stop a runaway test, making that test
        ...
        if any(issubclass(asyncio.CancelledError, t) for t in types):
            raise TypeError(
                "raises() cannot catch asyncio.CancelledError: voci uses cancellation to "
...
def raises(expected, func=None, *args, match=None, **kwargs):
    ...
    An `expected` that would catch `asyncio.CancelledError` raises `TypeError`: voci enforces
