...
`_marks.py` stay flat at the package root — `fastapi.py`'s import path (`velox.fastapi`) is
...
velox never depends on `velox-migrate` — see that distribution's own CLAUDE.md.
