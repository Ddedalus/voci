"""The collection index under `rootdir/.velox_cache`: what a file collected last time it was
actually imported, kept as long as its `(mtime_ns, size)` haven't changed since.

Written alongside the run cache (`velox._cache`) at the end of any run that collected for real.
...
from velox._collection.collect import CollectionResult, display_path
...
#: Bumped when the payload's shape changes, exactly like `velox._cache`'s own -- a file written
...
"""Rootdir-relative path (as `str`, matching `velox._cache`'s own keys) -> what it collected."""
