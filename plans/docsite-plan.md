...
Internal working document. Plans a narrative-docs-plus-reference site for voci, structured and
...
- **Migrating from pytest** - dedicated documentation on migrating existing test suites. Covers usage of voci-migrate and common manual edit patterns.
...
Reference               (mkdocstrings, one page per __init__.py export group)
  ...
  reference/fastapi.md     voci.fastapi: client, lifespan, uninstall
  reference/cli.md         generated from `voci --help` (see below; not mkdocstrings — argparse
...
About
  ...
  about/alternatives.md    the one neutral pytest comparison from README.md, not expanded
                            ("Don't argue with pytest" — voci-docs skill)
```

Every guide/how-to page must pass the voci-docs skill's rules.
...
Same mechanism as FastAPI: `mkdocstrings[python]`, `::: voci.fixture`-style directives, reading docstrings that already follow the voci-docs register. `filters: ['!^_']` to keep private names out, `show_root_heading`, `merge_init_into_class`, `signature_crossrefs` — 

Configured under `[project.plugins.mkdocstrings.handlers.python.options]` in `zensical.toml`, per
[Zensical's mkdocstrings docs](https://zensical.org/docs/setup/extensions/mkdocstrings/). voci's
...
autodoc path. Generate it the way `voci/_assertions/_vendor/` is generated: a script
(`scripts/gen_cli_reference.py`) that imports `voci.cli.build_parser()` and renders its help text into the page, plus a `--check` mode wired into `just docs check.
