...
Internal working document. Answers "what would it take to run trio tests under velox", with a
design for the cheap option and a costed dismissal of the expensive one.

velox's execution model is asyncio all the way down by deliberate choice
...
| How is a trio test identified? | An explicit `@velox.trio` mark. A trio coroutine function is indistinguishable from an asyncio one by `inspect`, so velox has to be told |
| Is trio a velox dependency? | No. Optional extra `velox-test[trio]`, imported lazily; using the mark without trio installed is a collection error |
...
`trio.run` needs to own its thread. The one place velox already hands a test its own thread with
full context propagation is the sync-test branch of the call phase
([run.py:291](../velox/_run/run.py#L291)), dispatching into
`_capture.ContextPropagatingExecutor` ([capture.py:356](../velox/_builtins/capture.py#L356)).
That executor copies the calling task's `contextvars.Context` at submit time, which is exactly
what every velox subsystem keyed off a ContextVar needs: the capture sink, the log routing
handler (already documented as running on worker threads,
[capture.py:180](../velox/_builtins/capture.py#L180)), the assertion hooks, `test_info`. A trio
...
([run.py:1006](../velox/_run/run.py#L1006)), so a trio test never waits for a thread while its own
timeout budget burns. It occupies one concurrency slot and one worker thread for its duration,
same as a sync test.

## 3. Build order

**Step 1 — the mark.** `velox.trio`, a bare decorator folding `trio: bool = False` into `Marks`
([_marks.py:98](../velox/_marks.py#L98)), alongside `solo` and `isolated`. Exported from
`velox/__init__.py`. Collection rejects the combination `@velox.trio` on a non-`async def`
function, and rejects `@velox.trio` when `import trio` fails, both as ordinary `CollectionError`s
([collect.py:127](../velox/_collection/collect.py#L127)) naming the install extra.
...
function. `track_sync_call` ([safety.py:254](../velox/_run/safety.py#L254)) is reused unchanged, so
...
pass the effective budget (per-test `@velox.timeout` mark, else `--timeout`) into the wrapper and
...
([run.py:317](../velox/_run/run.py#L317)) reports `timeout` with no special-casing. Both deadlines
...
- Extend the `raises()` cancellation guard ([raises.py:73](../velox/_assertions/raises.py#L73)) to
...
- [matrix.py:1013](../velox-migrate/velox_migrate/matrix.py#L1013): `pytest-trio` moves from
  `VX323` ("velox runs tests on asyncio") to `VX320` with the `@velox.trio` recipe. `pytest-tornasync`
  stays `VX323`.

## 4. What this does not give you

**trio-native async fixtures.** Fixtures are constructed by `_di` on the asyncio loop
([runtime.py:315](../velox/_di/runtime.py#L315)) and their values are handed across the thread
...
bound to the trio run that no longer exists by the time the test starts, is not — and velox cannot
...
thread, not the shared loop, so [safety.py](../velox/_run/safety.py)'s heartbeat correctly stays
quiet. `stuck_calls` is the diagnostic instead.

**Threads spawned by `trio.to_thread.run_sync`** come from trio's own limiter, outside velox's
...
`velox.Skipped` / xfail through `trio.run`, timeout expiry, fixture injection, capture and log
...
The mechanical parts map 1:1 — TaskGroup, timeouts, subprocess ([isolated.py:143](../velox/_run/isolated.py#L143)),
Condition, Event — and `ScopeStore`'s `asyncio.Future` single-flight
([runtime.py:114](../velox/_di/runtime.py#L114)) becomes an Event plus a stored result in half a
...
([run.py:319-345](../velox/_run/run.py#L319-L345)) — trio forbids all of that, since `Cancelled`
...
tests: `loop.set_default_executor` ([run.py:1023](../velox/_run/run.py#L1023)) has no trio
analogue, so `ContextPropagatingExecutor` needs a parallel implementation and a user's
`asyncio.to_thread` stops landing in velox's pool. Third, the abort path — `asyncio.all_tasks`,
`set_exception_handler`, closing the loop under pending tasks
([run.py:1039-1105](../velox/_run/run.py#L1039-L1105)) — is asyncio escape hatches with no anyio
...
the documented performance story, and it is 4–6 weeks to rewrite velox's subtlest subsystem for a
