# pytest → voci migration: the problem
...
implementation. It is written against voci as it exists in this tree, plus the near-term entries in
...
voci has no runtime pytest compatibility layer and will not grow one: no `conftest.py`, no
...
- **Output**: a voci suite where every dependency is an imported object named in a `Depends()`
...
1. **Wiring translation.** Mechanical, statically decidable given pytest's own resolution answers,
   produces a very large diff, and is finished when the suite collects under voci.
2. **Concurrency adaptation.** A pytest suite is written under assumptions voci deletes: one test
   at a time, in file order, with process-global state (env, `sys.modules`, monkeypatched
   attributes, warning filters, the CWD) effectively private to whichever test is running. voci
...
- **Fixtures are objects, imported by name.** `@voci.fixture()` returns a `Fixture`; `Depends()`
...
- **`class Test*` is pure namespacing**: its `test_*` methods collect as
  ...
  voci provides `setup_method`/`teardown_method` or `unittest.TestCase`, and a class carrying
  either is a collection error naming what to move into a fixture.
- **Configuration is `[tool.voci]` in `pyproject.toml`** with six recognized keys — `testpaths`,
...
async, sync- and async-generator bodies) → `@voci.fixture()`; `parametrize` → `@voci.parametrize`
(with `ids`); `@pytest.fixture(params=...)` → `@voci.fixture(params=...)`, `request.param` → the
fixture body's own `param` argument; `skip`/`skipif`/`xfail` marks; custom marks →
`@voci.tag(...)`, selectable with `-m`; `tmp_path`/`tmp_path_factory`; `capsys` → `voci.capture`;
`caplog` → `voci.log_records`; `pytest.raises` (including `match=` semantics, `re.search`, same
escaping gotcha); `pytest.approx` for scalars; `@pytest.mark.asyncio`/`anyio` marks and
`event_loop` fixtures → deleted; `pytest-timeout` → `@voci.timeout(...)`;
`@pytest.fixture(autouse=True)` and `@pytest.mark.usefixtures(...)` → `voci.use(...)` on the
...
decision on how much implicit specialization voci wants at all
...
capture (`capfd` — voci replaces `sys.stdout`/`sys.stderr`, so a C extension or subprocess writing
...
- Reproducing conftest scoping by static analysis is reimplementing the exact machinery voci
...
voci's dependency edges are hard-wired at import time: `Depends(session)` names one object,
forever. pytest's edges are resolved per test, late. Two common patterns exploit the difference,
and both collapse into the same problem — **one pytest fixture corresponds to N voci fixture
...
1. **Conftest override / shadowing.** `tests/conftest.py` defines `settings`; `tests/integration/
   ...
   under `integration/`. In voci, redefining the leaf changes nothing for its dependents. Correct
...
2. **`indirect=True` parametrization.** `@pytest.mark.parametrize("backend", [...], indirect=True)`
   ...
   `@pytest.fixture(params=...)`, which maps directly onto `@voci.fixture(params=...)` (§3), but
...
site. `voci.use(...)` is the counterpart: a declaration on the container rather than on each test,
...
holds: pytest orders autouse fixtures before others at the same scope, and `voci.use` puts its
...
`request` is pytest's escape hatch and has no voci counterpart. Each use is a different problem:
...
- `request.node` (name, own marks, `nodeid`), `request.config` (options, ini values),
  ...
  `voci.test_info` (`id`, `tags`, `timeout`, `worker`). `request.config.getoption(...)` reaching a
  custom `pytest_addoption` flag has no path at all, because voci has no plugin CLI surface.

### 4.5 Output layout and the import graph

voci suites import fixtures from ordinary modules, so the tool has to *invent a file layout*. It
...
- Name collisions become real: two `conftest.py` files defining `client` are fine in pytest and
  need distinct importable names in voci.
- Fixture modules import application code; test modules import fixture modules; specialized chains
  (§4.2) import each other. Cycles are reachable, and voci resolves imports rootdir-relative with
  namespace-package lookup, no `__init__.py` required — but relative imports between test modules
  do not work, because modules are imported under synthetic `voci_tests.*` names.
...
| pytest | voci | Trap |
|---|---|---|
| `@pytest.mark.parametrize(...)` | `@voci.parametrize(...)` | `pytest.param(v, marks=..., id=...)` has no voci spelling: per-case marks do not exist. A per-case `xfail` must become a separate test or a body-level branch. Generated ids differ (voci renders `bool`/`str`/`int`/`None`/enum, else `argname{index}`; floats, bytes, tuples diverge). |
| `@pytest.mark.skipif(cond, reason=)` | `@voci.skipif(cond, reason=)` | voci takes a `bool` or a zero-arg callable; pytest also accepts a *string expression*. Conditions that read module state at import time change meaning if wrapped in a callable (voci evaluates callables at run time, not collection). |
| `@pytest.mark.xfail(...)` | `@voci.xfail(reason, strict=, raises=)` | pytest's `condition` first argument and `run=False` have no equivalent; `run=False` must become a skip. `xfail_strict` from ini has to be materialized per mark. |
| `@pytest.mark.usefixtures("a","b")` | `voci.use(a, b)` | Applies to the whole module, so a mark on one test in a file of many needs the others checked, or the fixture moved to its own module. |
| `pytest.raises(E, match=)` | `voci.raises(E, match=)` | voci refuses to catch `asyncio.CancelledError` (it is the timeout mechanism). The legacy call form `pytest.raises(E, fn, *args)` has no equivalent. |
| `pytest.approx(x)` | `voci.approx(x)` | **Scalars only.** Sequence/dict/numpy comparisons have no equivalent and must be flagged, not rewritten. |
| `capsys.readouterr()` | `voci.capture` | **Semantics differ.** `readouterr()` returns a snapshot *and clears the buffer*; `Capture.out`/`.err` are live, cumulative, never cleared. Any test calling `readouterr()` twice changes meaning under a rename. `capfd`/binary variants have no equivalent at all. |
| `caplog` | `voci.log_records` | `caplog.set_level(...)` is a statement with automatic restore at test end; `LogRecords.set_level(...)` returns a context manager and does nothing until entered — so the rewrite has to restructure the test body. `caplog.records`/`.messages` map cleanly; `.text`, `.record_tuples`, `.clear()`, and the handler-level filtering surface do not. Logger levels are process-global, so this is also a §6 hazard. |
...
| `pytest.skip()`, `pytest.fail()`, `pytest.xfail()` inside a body | — | **The most dangerous rename in the whole project.** `voci.skip("reason")` is a decorator factory: as a bare statement it evaluates, returns a function, and does nothing. A blind `pytest.` → `voci.` rewrite converts a conditional skip into a test that silently runs. Must be flagged, never rewritten. |
| `pytest.importorskip("mod")` | — | No equivalent. Closest faithful form is a module-level import guard plus `@voci.skipif`. |
...
| `unittest.mock.patch` | left as-is | Keeps working. The decorator form is found at collection and scheduled solo; the context-manager form is invisible until it runs, so voci refuses it at install time and the test fails until it carries `@voci.solo` — the codegen should add that mark at every such site. |
...
- **Process-global mutation under concurrency.** The whole `monkeypatch` surface (`setenv`,
  ...
  are private to the running test; under voci they are visible to every test in flight. The voci
  answers are `@voci.solo` (correct, costs wall clock), `@voci.isolated` (a subprocess, correct
...
- **Fixture teardown timing.** pytest tears down a module-scoped fixture when the last test in that
  module finishes, in serial order; voci tears down when the last holder releases, with tests from
...
Ranked roughly by how often they appear in the async-Python suites voci targets:
...
`filterwarnings`, whose specs voci reads the same way. What has
no home: `addopts` (each flag needs individual treatment), `markers` registration (voci tags need
no declaration), `asyncio_mode`, `xfail_strict` (must be pushed down into each `@voci.xfail`),
`log_cli`, `console_output_style`, `required_plugins`, `pytest_plugins`, and every option added by
`pytest_addoption` in a conftest. Unknown `[tool.voci]` keys are a hard
error, so the tool cannot leave anything speculative in the generated config.

The environment matters too: `env = {...}` in `[tool.voci]` is suite-wide, which is the natural
...
key on pytest node ids. voci ids are already close (`path::qualname[case]`), and the differences
are: class-based ids lose their middle segment, and generated parametrize ids diverge for anything
that is not `bool`/`str`/`int`/`None`/enum. Since voci's `@voci.parametrize` accepts an explicit
...
4. **Where do the fixtures a `voci.use(...)` line names live**, given the `__init__.py` that
