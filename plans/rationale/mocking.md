...
**voci detects patching and schedules around it rather than shipping a patch API of its own.**
...
running at that moment sees it. A voci-branded patch decorator that also had to run solo would be
`unittest.mock` with a different import line and a migration cost, so voci delegates the patching
...
adopted voci for, and should read that off the report rather than a stopwatch.
...
exactly the failure shape voci refuses elsewhere. `real_function` unwraps to the function actually
...
`unittest.mock` fills its mock arguments in ahead of voci's keyword arguments, positionally, so
...
`@voci.parametrize` already does, so those names join the same set of externally supplied
arguments.
...
message names: `@voci.solo` for the test that patches, `@voci.isolated` for a target with no
