...
```json
{"code": "VC401", "message": "`monkeypatch.setenv` writes an environment variable every test in flight can see.",
 "file": "conftest.py", "line": 43, "function": "patched_env", "tests": [ ... 45 node ids ... ]}
{"code": "VC402", "message": "`os.environ.pop(...)` writes the process environment.",
...
rule sends it to the seam: 45 `@voci.solo` marks would be a serial suite with extra syntax.

In the converted tree the file is `fixtures.py`, and this fixture is still pytest-spelled: voci
...
`LEFTOVER` was teardown for a variable some *test* set; that test is its own `VC402` finding lower
...
(`import voci`, `from voci import Depends`, no annotation on injected parameters):
...
```json
{"code": "VC401", "message": "`monkeypatch.setattr` rebinds an attribute every test in flight shares.",
...
{"code": "VC401", "message": "`monkeypatch.chdir` changes the working directory, so this test needs isolation.",
...
wins: `chdir` is `@voci.isolated`, which subsumes the `setattr` finding's `@voci.solo`.
...
`monkeypatch` has no voci counterpart, so the rebinding is hand-ported; the context-manager patch
is legal here only because the test is `@voci.isolated` — the guard admits a `with mock.patch(...)`
...
resolver in as a fixture — since one `@voci.solo` still drains the run for that test's duration.

## 3 — A contended resource, not a global write

```json
{"code": "VC413", "message": "`port=5432` is a fixed port number.", "file": "test_concurrency.py", "line": 22}
```
...
```python
@voci.fixture(exclusive="pg-5432")
def server():
