"""Tests for voci_migrate.audit: the classification of a whole suite, end to end.
...
from voci_migrate.audit import Audit
from voci_migrate.audit.findings import SEVERITY
from voci_migrate.matrix import Disposition
...
def test_the_dump_and_the_sources_are_both_read(hazards: Audit) -> None:
    ...
    assert "VC006" in codes(hazards)
    assert "VC223" in codes(hazards)


@pytest.mark.parametrize(
    "code",
    [
        "VC003",  # a class-scoped fixture
        "VC006",  # an over-budget override
        "VC010",  # usefixtures on one test of a module
        "VC011",  # a literal getfixturevalue
        "VC012",  # a computed getfixturevalue
        "VC013",  # an unconditional finalizer
        "VC014",  # a conditional one
        "VC015",  # request.node
        "VC016",  # a custom command-line flag
        "VC019",  # setup_method
        "VC020",  # unittest.TestCase
        "VC022",  # a conftest hook
        "VC023",  # pytest_addoption
        "VC102",  # a per-case mark
        "VC103",  # a string skipif condition
        "VC105",  # a conditional xfail
        "VC106",  # xfail(run=False)
        "VC108",  # a per-test warning filter
        "VC113",  # two timeouts on one test
        "VC202",  # readouterr twice
        "VC203",  # capfd
        "VC205",  # caplog.set_level
        "VC210",  # a raises object stashed rather than entered or called
        "VC223",  # pytest.xfail() as a statement
        "VC215",  # importorskip
        "VC216",  # recwarn and pytest.warns
        "VC217",  # a mock.patch decorator
        "VC218",  # a mock.patch context manager
        "VC221",  # approx over a generator
        "VC222",  # caplog.handler
        "VC305",  # addopts
        "VC307",  # filterwarnings in the ini file
        "VC308",  # log_cli
        "VC401",  # monkeypatch
        "VC402",  # an os.environ write
        "VC403",  # a warnings filter
        "VC404",  # a logging level
        "VC405",  # sys.modules surgery
        "VC406",  # os.chdir
        "VC407",  # the locale
        "VC409",  # a second event loop
        "VC410",  # a blocking call in a coroutine
        "VC411",  # module-level state
        "VC412",  # a seeded generator
        "VC413",  # a fixed resource
    ],
)
...
def test_a_blocking_call_is_only_a_hazard_inside_a_coroutine(hazards: Audit) -> None:
    blocking = by_code(hazards, "VC410")
...
def test_an_override_is_refused_by_its_fan_out_and_allowed_by_a_bigger_budget(
    ...
    refused = by_code(audit_of(HAZARDS, version), "VC006")
    (finding,) = refused

    assert finding.detail["fan_out"] == 7
    assert finding.disposition is Disposition.REFUSED
    assert codes(audit_of(HAZARDS, version, budget=10)) >= {"VC005"}
    assert "VC006" not in codes(audit_of(HAZARDS, version, budget=10))
...
def test_a_hazard_in_a_test_body_reaches_that_test_alone(hazards: Audit) -> None:
    in_a_test = [
        finding
        for finding in by_code(hazards, "VC402")
...
def test_a_hazard_in_an_autouse_fixture_reaches_the_tests_that_inherit_it(hazards: Audit) -> None:
    ...
    (in_a_fixture,) = [
        finding for finding in by_code(hazards, "VC402") if finding.site.function == "patched_env"
    ]
...
def test_two_directories_declaring_the_same_autouse_name_are_told_apart(hazards: Audit) -> None:
    # Both conftests define `patched_env`, and the `voci.use(...)` line for each goes where its
    ...
    sites = {
        ...
        for finding in by_code(hazards, "VC008")
...
def test_an_indirectly_parametrized_method_counts_its_own_cases(hazards: Audit) -> None:
    ...
    (indirect,) = by_code(hazards, "VC007")
...
def test_a_finding_about_the_suite_names_no_test(hazards: Audit) -> None:
    (log_cli,) = [
        finding for finding in by_code(hazards, "VC308") if finding.detail["setting"] == "log_cli"
    ]
...
def test_an_override_within_budget_is_mechanical(fixtures: Audit) -> None:
    (override,) = by_code(fixtures, "VC005")
...
def test_an_autouse_fixture_is_reported_where_its_declaration_goes(fixtures: Audit) -> None:
    autouse = {finding.detail["node"]: finding for finding in by_code(fixtures, "VC008")}
...
def test_a_literal_dynamic_lookup_converts_and_a_computed_one_does_not(hazards: Audit) -> None:
    (literal,) = by_code(hazards, "VC011")
    (computed,) = by_code(hazards, "VC012")
...
def test_a_hazard_in_a_fixture_reaches_the_tests_that_request_it(hazards: Audit) -> None:
    (chdir,) = [finding for finding in by_code(hazards, "VC401") if "chdir" in finding.message]
...
def test_the_blind_spots_are_the_constructs_no_audit_can_see(hazards: Audit) -> None:
    ...
    assert spots == {"VC035", "VC414", "VC415", "VC416"}
