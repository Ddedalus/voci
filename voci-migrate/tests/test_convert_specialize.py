"""Tests for voci_migrate.convert.specialize: one overridden name becoming two objects.
...
from voci_migrate.audit import wiring
from voci_migrate.convert import plan, specialize
...
def test_a_test_below_the_overriding_directory_reads_the_same_copy(tree: Path) -> None:
    # voci reads no directory at run time, so a test two levels down has to be pointed at the
...
def test_the_refusal_quotes_the_fan_out_the_audit_computed(version: str) -> None:
    result = conversion_of(OVERRIDES, version, budget=2)
    (refusal,) = [finding for finding in result.plan.refusals if finding.code == "VC006"]
...
def test_an_override_of_an_autouse_fixture_leaves_both_definitions_alone(version: str) -> None:
    # A `voci.use(...)` names one object for a directory, so an autouse fixture an override
...
def test_a_declaration_inside_the_subtree_names_the_copy(tree: Path) -> None:
    ...
    assert "voci.use(client_integration)" in body
...
def test_specialization_is_the_row_the_matrix_says_it_is() -> None:
    assert "VC005" not in plan.DEFERRED
