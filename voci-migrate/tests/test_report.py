"""Tests for voci_migrate.report: the JSON payload, the markdown report and the terminal summary.
...
from voci_migrate.report import (
    FINDINGS_VERSION,
    markdown,
    payload,
    terminal,
...
CODE_ROW = re.compile(r"^VC\d{3}\s")
...
def test_a_caveat_names_the_marker_the_converted_source_carries() -> None:
    ...
    assert "`VOCI-TODO[scope]`" in caveats.split("## Concurrency hazards")[0]
...
def test_the_report_lists_the_fixtures_to_annotate_before_anything_is_converted() -> None:
    ...
    assert "3 of 4 fixtures defined here state none" in section
...
def test_the_terminal_summary_quotes_the_worklist_in_one_line() -> None:
    ...
    assert "types: 3 of 4 fixtures state no return type, costing 3 injected" in summary
