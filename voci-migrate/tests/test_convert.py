"""Tests for voci_migrate.convert: the rewrite, end to end and in its parts.

The end-to-end tests are the ones that matter. A conversion is only correct if its output runs, so
the mechanical corpus suite is converted into a temporary tree, executed under voci, and compared
...
from voci_migrate import audit, convert, matrix, model
from voci_migrate.convert import annotate, config, layout, markers, parametrize, plan, wiring
from voci_migrate.convert.layout import Import
from voci_migrate.report import conversion as conversion_report
...
def ids_under(runner: list[str], tree: Path) -> list[str]:
    ...
    Both runners list one id per line under `--collect-only -q`; voci follows a skipped test's id
...
VOCI = [sys.executable, "-c", "import sys; from voci.cli import main; sys.exit(main())"]
...
def test_the_converted_mechanical_suite_passes_under_voci(version: str, tmp_path: Path) -> None:
    tree = tmp_path / MECHANICAL
    converted(MECHANICAL, version, tree)

    completed = subprocess.run(
        [*VOCI, "--serial", str(tree)], capture_output=True, text=True, check=False, cwd=tree
    )
...
def test_the_converted_declarations_suite_passes_under_voci(version: str, tmp_path: Path) -> None:
...
def test_the_converted_declarations_suite_keeps_every_pytest_node_id(
    ...
    assert ids_under(VOCI, tree) == ids_under(PYTEST, CORPUS / DECLARATIONS)
...
def test_the_converted_bodies_suite_passes_under_voci(version: str, tmp_path: Path) -> None:
...
def test_the_converted_bodies_suite_keeps_every_pytest_node_id(
    ...
    assert ids_under(VOCI, tree) == ids_under(PYTEST, CORPUS / BODIES)
...
def test_a_patch_applied_as_a_decorator_keeps_it_and_is_marked_by_nothing(
    ...
    # voci finds a decorator's patching on the function object at collection and schedules that
...
def test_the_converted_overrides_suite_passes_under_voci(version: str, tmp_path: Path) -> None:
...
def test_the_converted_overrides_suite_keeps_every_pytest_node_id(
    ...
    assert ids_under(VOCI, tree) == ids_under(PYTEST, CORPUS / OVERRIDES)
...
def test_the_converted_classes_suite_passes_under_voci(version: str, tmp_path: Path) -> None:
    # The bar for lifting: a voci test class has no fixtures, so every factory written in one is
...
def test_the_converted_classes_suite_keeps_every_pytest_node_id(
    ...
    assert ids_under(VOCI, tree) == ids_under(PYTEST, CORPUS / CLASSES)
...
def test_the_converted_parametrize_suite_passes_under_voci(version: str, tmp_path: Path) -> None:
...
def test_the_converted_parametrize_suite_keeps_every_pytest_node_id(
    ...
    assert ids_under(VOCI, tree) == ids_under(PYTEST, CORPUS / PARAMETRIZE)
...
def test_an_indirect_marks_values_become_the_fixtures_own_cases(
    ...
    assert '@voci.fixture(params=["mysql", "sqlite"], ids=["mysql", "sqlite"])' in fixtures
...
def test_a_hook_that_built_cases_leaves_them_written_on_each_test(
    ...
    assert '@voci.parametrize("width,height", [(2, 3), (5, 8)], ids=["small", "large"])' in body
    assert '@voci.parametrize("letter", ["a", "b"], ids=["a", "b"])' in body
    assert "VOCI-TODO[VC024]" in body
...
def test_an_indirect_mark_on_a_fixture_with_cases_of_its_own_refuses_the_test(
    ...
    assert [f.code for f in result.plan.refusals if f.code == "VC029"] == ["VC029"]
    ...
    assert "VOCI-TODO[VC029]" in body
...
def test_a_generated_value_with_no_literal_spelling_refuses_the_test() -> None:
    ...
    assert [f.code for f in result.plan.refusals] == ["VC031"]
...
def test_the_converted_suite_keeps_every_pytest_node_id(version: str, tmp_path: Path) -> None:
    # §9's promise, and why `@voci.parametrize` is emitted with pytest's own ids: a CI config, a
    # flaky-test dashboard or a `--last-failed` habit that names an id keeps working.
    tree = tmp_path / MECHANICAL
    converted(MECHANICAL, version, tree)

    assert ids_under(VOCI, tree) == ids_under(PYTEST, CORPUS / MECHANICAL)
...
def test_stacked_parametrize_marks_are_reversed_so_their_composed_ids_hold(
    ...
    # first in the id; voci reads its own list outermost-first. Reversing is what makes the two
    ...
    assert stacked.index('@voci.parametrize("inner"') < stacked.index('@voci.parametrize("outer"')
...
def test_an_axis_of_its_own_carries_pytests_ids_verbatim(version: str, tmp_path: Path) -> None:
    ...
    # ids and voci's diverge, and copying pytest's closes the gap rather than documenting it.
    tree = tmp_path / MECHANICAL
    converted(MECHANICAL, version, tree)

    collected = ids_under(VOCI, tree)
...
def test_a_name_the_suite_defines_twice_is_not_one_a_parameter_can_carry(
    ...
    assert "VOCI-TODO[VC028]" in body
...
def test_a_request_the_rewrite_cannot_empty_leaves_the_finalizer_where_it_was(
    ...
    assert "VOCI-TODO[VC014]" in body
...
def test_a_class_scoped_fixture_widens_and_says_so(version: str, tmp_path: Path) -> None:
    ...
    assert '@voci.fixture(scope="module")' in body
    assert "VOCI-TODO[VC003]" in body
...
def test_a_marker_names_the_matrix_code() -> None:
    assert markers.comment("VC102").startswith("# VOCI-TODO[VC102]: ")


def test_a_marker_is_written_once_however_often_it_is_asked_for() -> None:
    module = cst.parse_module("def test_it():\n    pass\n")

    once = markers.mark(module, {"test_it": ["VC102"]})
    twice = markers.mark(once, {"test_it": ["VC102"]})

    assert once.code.count("VOCI-TODO") == 1
...
def test_a_marker_on_a_method_is_indented_with_it() -> None:
    ...
    marked = markers.mark(module, {"TestIt.test_one": ["VC102"]})

    assert "\n    # VOCI-TODO[VC102]" in marked.code


def test_a_marker_about_the_module_lands_after_its_docstring() -> None:
    module = cst.parse_module('"""Doc."""\n\nimport pytest\n')

    marked = markers.mark(module, {"": ["VC022"]})

    lines = marked.code.splitlines()
    assert lines[0] == '"""Doc."""'
    assert "VOCI-TODO[VC022]" in lines[2]
...
def test_configuration_is_translated_into_the_table_voci_reads(version: str) -> None:
    result = conversion_of(HAZARDS, version)
...
def test_a_parameter_asked_for_by_name_goes_behind_a_star_where_nothing_else_is_valid() -> None:
    ...
    # go, at either end. voci binds by keyword, so the new parameter becomes keyword-only.
...
def test_a_positional_only_parameter_refuses_the_definition_under_its_own_row() -> None:
    # voci binds by keyword, so the parameter can never be given the fixture, and the signature
    ...
    assert result.refused == (("test_x", "VC036"),)
...
def test_a_test_needing_no_injection_does_not_import_depends(version: str, tmp_path: Path) -> None:
    ...
    assert "import voci" in body
    assert "Depends" not in body


def test_a_capsys_use_no_rule_rewrites_refuses_the_test_rather_than_stranding_it() -> None:
    # `capsys` is renamed with its parameter because `voci.capture` is a different object. A use
    ...
    work = plan.FileWork(
        ...
        tests=(
            plan.TestWork(
                qualname="test_x",
                injections=(plan.Injection("capsys", "capture", "voci.capture"),),
            ),
        ),
    )
    ...
    assert result.refused == (("test_x", "VC202"),)
    assert result.module.code == source


def test_a_carried_case_list_replaces_what_the_decorator_said_about_cases() -> None:
    # pytest tolerates an `ids=` with no `params=` beside it, and voci raises on one. The cases
...
"""
    ...
    assert '@voci.fixture(params=["mysql"], ids=["mysql"])' in result.module.code
...
def test_a_hazard_is_converted_rather_than_refused() -> None:
    # A hazard is what concurrency changes, not what syntax changes: `@voci.solo` is the answer and
    ...
    hazard = audit.Finding(code="VC402", message="writes the environment")

    assert not plan.refused(hazard)


def test_a_norecursedirs_pattern_is_dropped_rather_than_carried_as_a_name() -> None:
    # pytest matches `norecursedirs` as fnmatch patterns; voci's `ignore` compares directory names
...
def test_the_converted_typed_suite_passes_under_voci(version: str, tmp_path: Path) -> None:
    # The bar for this suite specifically: every type it writes is named through an import that
    # exists only for a type checker, so an annotation voci evaluated would not import at all.
    tree = tmp_path / TYPED
...
def test_every_recoverable_shape_in_the_typed_suite_is_written_as_a_type(
    ...
    # `-> Iterator[X]` is unwrapped, exactly as `@voci.fixture()`'s own overloads unwrap it.
    ...
    # A built-in's type comes from voci, since the suite's own sources say nothing about it.
    assert "tmp_path: Annotated[Path, Depends(voci.tmp_path)]" in top
    assert "capture: Annotated[voci.Capture, Depends(voci.capture)]" in top
...
def test_a_written_builtin_type_matches_what_voci_declares() -> None:
    """Every `BUILTIN_TYPES` row names the object voci's own fixture is declared to return.

    The table is written down because voci is not a dependency of this tool and the suite's
    sources say nothing about what `tmp_path` returns. This is what keeps it from drifting: it
    resolves both spellings to objects and compares those, so a rename or a retype in voci fails
    here rather than in somebody's converted suite.
    """
    # The one place this package reads voci, and a test rather than the tool: `voci-migrate`
    # converts a suite *for* voci and must keep working without it installed.
    import inspect

    import voci
    from voci._builtins import fixtures as declarations

    written_in = {"voci": voci, "Path": Path}
    ...
    for argname, (_, reference) in plan.BUILTINS.items():
        declared = getattr(voci, reference.removeprefix("voci.")).func.__annotations__["return"]
...
def test_every_builtin_with_a_voci_counterpart_has_a_type_to_write() -> None:
...
_CAPSYS = plan.Injection(
    was="capsys",
    param="capture",
    reference="voci.capture",
    annotation="voci.Capture",
    retypes=True,
)
_TMP_PATH = plan.Injection(
    was="tmp_path",
    param="tmp_path",
    reference="voci.tmp_path",
    annotation="Path",
    needs=(annotate.TypeImport("pathlib", "Path"),),
)


def test_a_builtin_is_written_with_voci_s_type_and_not_the_one_pytest_gave_it() -> None:
    # `capsys` becomes a `voci.Capture`, an object with different methods -- so unlike a fixture
    ...
    assert "capture: Annotated[voci.Capture, Depends(voci.capture)]" in result.module.code
    assert "tmp_path: Annotated[Path, Depends(voci.tmp_path)]" in result.module.code
...
def test_a_builtin_voci_hands_back_unchanged_keeps_the_annotation_its_author_wrote() -> None:
    # voci's `tmp_path` is the same `pathlib.Path` pytest's was, so the author already answered
    ...
    assert "tmp_path: Annotated[pathlib.Path, Depends(voci.tmp_path)]" in result.module.code
    assert "import pathlib" in result.module.code
    assert "TYPE_CHECKING" not in result.module.code


def test_only_the_builtins_voci_changes_the_object_of_replace_an_annotation() -> None:
...
def test_a_builtin_type_the_consumer_already_binds_is_imported_under_an_alias() -> None:
    ...
    assert typed == annotate.Typed(
        annotation="voci_Path", imports=(annotate.TypeImport("pathlib", "Path", "voci_Path"),)
    )
...
def test_a_builtin_voci_has_no_counterpart_for_is_no_worklist_row() -> None:
    ...
    work = plan.FileWork(
        ...
        tests=(
            plan.TestWork(
                qualname="test_x",
                injections=(
                    plan.Injection("capsys", "capture", "voci.capture"),
