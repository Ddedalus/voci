"""Tests for voci_migrate.convert.declarations: where a `voci.use(...)` goes, and what it names.
...
from voci_migrate.convert import declarations
...
def test_a_class_node_declares_nothing() -> None:
    # `voci.use(...)` covers a module or a package, and a fixture written in a class body binds no
...
def test_every_directory_between_a_declaration_and_a_test_becomes_a_package(
    ...
    # voci walks up from the test file and stops at the first directory that is not a package, so
...
def test_an_empty_package_the_suite_already_has_is_written_like_any_other_file(
    ...
    assert (
        (tree / "api" / "__init__.py").read_text(encoding="utf-8").endswith("voci.use(api_seed)\n")
    )
...
def test_a_declaration_naming_the_modules_own_fixture_lands_after_it() -> None:
    ...
    module = cst.parse_module("import voci\n\n\n@voci.fixture()\ndef seed():\n    pass\n")
...
def test_a_second_pass_declares_nothing_twice() -> None:
    ...
    assert once.code.count("voci.use") == 1
