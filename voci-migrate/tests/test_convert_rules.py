"""Tests for voci_migrate.convert.rules: what each numbered rule writes, and what it declines.
...
from voci_migrate.convert import rules
...
def test_a_disabled_code_is_left_out_of_the_set() -> None:
    remaining = rules.enabled(["VC101", "vc204 "])

    assert [rule.code for rule in remaining] == [
        code for code in (rule.code for rule in rules.RULES) if code not in ("VC101", "VC204")
    ]
...
def test_a_usefixtures_mark_goes_away() -> None:
    # The fixture it named is declared with `voci.use(...)` on the module, which is placed from
...
def test_x():
    pass
"""
    applied = _rewrite("VC009", before, after, _context("test_x"))

    assert _codes(applied) == ["VC009"]
...
def test_x():
    pass
"""
    assert _untouched("VC009", source, _context("test_x", blocked=["test_x"])) == ()
...
@voci.parametrize("value", [1, 2], ids=("1", "2"))
def test_x(value):
    assert value
"""
    applied = _rewrite(
...
@voci.parametrize("value", [1, 2], ids=("one", "two"))
def test_x(value):
    assert value
"""
    _rewrite("VC101", before, after, _context("test_x"))
...
@voci.parametrize("a,b", [(1, 2)], ids=("low",))
def test_x(a, b):
    assert a < b
"""
    _rewrite("VC101", before, after, _context("test_x"))
...
def test_x(value):
    assert value
"""
    applied = _rewrite("VC101", before, after, _context("test_x"))

    assert _codes(applied) == ["VC114"]
...
"""
    result = _apply(
        "VC101", source, _context("test_x", axis_ids={("test_x", "value"): ("1", "2", "3")})
    )

    assert "ids=" not in result.module.code


def test_a_mark_on_one_case_becomes_voci_case() -> None:
    before = """import pytest
...
@voci.parametrize("value", [1, voci.case(2, marks=voci.skip("no reason given"))])
def test_x(value):
    assert value
"""
    applied = _rewrite("VC101", before, after, _context("test_x"))

    assert _codes(applied) == ["VC102"]
...
@voci.parametrize(
    "value", [voci.case(2, marks=[voci.xfail("known"), voci.tag("slow")])]
)
def test_x(value):
    assert value
"""
...
def test_a_cases_xfail_mark_carries_the_suites_strict() -> None:
    ...
    as a top-level `@voci.xfail` does -- `_StrictXFail` (VC107) only patches decorators, so a
    mark nested inside `voci.case(..., marks=...)` must already carry it."""
    before = """import pytest
...
@voci.parametrize("value", [voci.case(2, marks=voci.xfail("known", strict=True))])
def test_x(value):
    assert value
"""
    applied = _rewrite("VC101", before, after, _context("test_x", xfail_strict=True))
...
@voci.parametrize("a,b", [voci.case(1, 2, marks=voci.skip("no reason given"))])
def test_x(a, b):
    assert a < b
"""
    _rewrite("VC101", before, after, _context("test_x"))
...
@voci.parametrize("value", [voci.case(2, marks=voci.skip("no reason given"))], ids=("two",))
def test_x(value):
    assert value
"""
    _rewrite("VC101", before, after, _context("test_x"))
...
def test_x(value):
    assert value
"""
    applied = _untouched("VC101", source, _context("test_x"))
...
"""
    applied = _rewrite(
        "VC007", before, after, _context("test_x", indirect={"test_x": frozenset({"value"})})
    )

    assert _codes(applied) == ["VC007"]
...
def test_the_cases_a_hook_produced_are_written_out_with_pytests_own_ids() -> None:
    ...
    assert width * height
"""
    after = """@voci.parametrize("width,height", [(2, 3), (5, 8)], ids=["small", "large"])
...
"""
    ...
    applied = _rewrite("VC024", before, after, _context("test_x", generated=generated))

    assert _codes(applied) == ["VC024"]
...
def test_a_generated_axis_whose_ids_no_one_position_explains_says_so() -> None:
    ...
    assert letter
"""
    after = """@voci.parametrize("letter", ["a", "b"])
def test_x(letter):
    assert letter
"""
...
def test_stacked_parametrize_marks_are_reversed() -> None:
    ...
    # written first in the case id; voci reads its own list outermost-first. Reversing is what
...
@voci.parametrize("b", [3, 4], ids=("3", "4"))
@voci.parametrize("a", [1, 2], ids=("1", "2"))
def test_x(a, b):
    assert a < b
"""
    _rewrite(
...
@voci.skipif(lambda: sys.platform == 'win32', reason="posix only")
def test_x():
    pass
"""
    applied = _rewrite("VC103", before, after, _context("test_x"))

    assert _codes(applied) == ["VC103"]
...
def test_x():
    pass
"""
    applied = _untouched("VC103", source, _context("test_x"))
...
# --- VC104 skip, skipif, xfail -----------------------------------------------------------------


def test_a_bare_skip_gets_the_reason_voci_requires() -> None:
    before = """import pytest
...
@voci.skip("no reason given")
def test_x():
    pass
"""
    applied = _rewrite("VC104", before, after, _context("test_x"))

    assert _codes(applied) == ["VC104"]
...
def test_x():
    pass
"""
    _rewrite("VC104", before, after, _context("test_x"))
...
@voci.skipif(sys.version_info < (3, 14), reason="too old")
def test_x():
    pass
"""
    _rewrite("VC104", before, after, _context("test_x"))
...
@voci.xfail("broken", strict=True, raises=ValueError)
def test_x():
    pass
"""
    _rewrite("VC104", before, after, _context("test_x"))
...
@voci.xfail("broken", condition=sys.platform == "win32")
def test_x():
    pass
"""
    applied = _rewrite("VC104", before, after, _context("test_x"))

    assert _codes(applied) == ["VC105"]
...
@voci.xfail("broken", condition=sys.platform == "win32", strict=True)
def test_x():
    pass
"""
    applied = _rewrite("VC104", before, after, _context("test_x"))
...
@voci.xfail("broken", condition=lambda: sys.platform == 'win32')
def test_x():
    pass
"""
    applied = _rewrite("VC116", before, after, _context("test_x"))

    assert _codes(applied) == ["VC116"]
    assert "`sys`" in applied[0].message
...
def test_x():
    pass
"""
    applied = _untouched("VC116", source, _context("test_x"))
...
def test_x():
    pass
"""
    applied = _untouched("VC106", source, _context("test_x"))

    assert _codes(applied) == ["VC106"]
...
def test_x():
    pass
"""
    applied = _untouched("VC104", source, _context("test_x"))

    assert _codes(applied) == ["VC113", "VC113"]
...
@voci.skipif(slow, reason="slow")
@voci.skipif(windows, reason="windows")
def test_x():
    pass
"""
    _rewrite("VC104", before, after, _context("test_x"))
...
def test_x():
    pass
"""
    applied = _rewrite("VC106", before, after, _context("test_x"))
...
def test_x():
    pass
"""
    assert _untouched("VC104", source, _context("test_x")) == ()
...
"""
    ...
    assert result.module.code == after
    assert _codes(result.applied) == ["VC104", "VC107"]
...
@voci.xfail("broken", strict=False)
def test_x():
    pass
"""
    assert _untouched("VC107", source, _context("test_x", xfail_strict=True)) == ()
...
def test_x():
    pass
"""
    applied = _rewrite("VC109", before, after, _context("test_x"))

    assert _codes(applied) == ["VC109"]
...
def test_x():
    pass
"""
    assert _untouched("VC109", source, _context("test_x")) == ()
...
async def test_x():
    pass
"""
    applied = _rewrite("VC110", before, after, _context("test_x"))

    assert _codes(applied) == ["VC110"]
...
async def test_x():
    pass
"""
    _rewrite("VC110", before, after, _context("test_x"))
...
def test_a_timeout_mark_becomes_voci_timeout() -> None:
    before = """import pytest
...
def test_x():
    pass
"""
    applied = _rewrite("VC111", before, after, _context("test_x"))

    assert _codes(applied) == ["VC111"]
...
def test_x():
    pass
"""
    applied = _untouched("VC111", source, _context("test_x"))

    assert _codes(applied) == ["VC111"]
    assert "positive" in applied[0].message


# --- VC108 filterwarnings ---------------------------------------------------------------------


def test_a_filterwarnings_mark_becomes_voci_filterwarnings() -> None:
    before = """import pytest
...
def test_x():
    pass
"""
    applied = _rewrite("VC108", before, after, _context("test_x"))

    assert _codes(applied) == ["VC108"]
...
@voci.filterwarnings("ignore::UserWarning", "error:.*legacy:DeprecationWarning")
def test_x():
    pass
"""
    applied = _rewrite("VC108", before, after, _context("test_x"))
...
"""
    ...
    assert _codes(applied) == ["VC108", "VC108"]
...
def test_x():
    pass
"""
    applied = _untouched("VC108", source, _context("test_x"))
...
def test_y():
    pass
"""
    applied = _rewrite("VC115", before, after, _context("test_x", "test_y"))

    assert _codes(applied) == ["VC115"]
...
class TestGroup:
    ...
    def test_x(self):
        pass
"""
    _rewrite("VC115", before, after, _context("TestGroup.test_x"))
...
def test_x():
    pass
"""
    _rewrite("VC115", before, after, _context("test_x"))
...
def test_x():
    pass
"""
    applied = _untouched("VC115", source, _context("test_x"))
...
def test_x():
    pass
"""
    applied = _untouched("VC115", source, _context("test_x", blocked=["test_x"]))
...
"""
    ...
    assert _codes(applied) == ["VC115", "VC113"]
...
"""
    applied = _rewrite(
        "VC011", before, after, _context(requested={"engine": {"settings": "settings"}})
    )

    assert _codes(applied) == ["VC011"]


def test_a_getfixturevalue_of_a_renamed_builtin_becomes_the_name_voci_binds() -> None:
...
"""
    ...
    assert _untouched("VC011", source, _context()) == ()
...
"""
    ...
    entries.clear()
"""
    applied = _rewrite("VC013", before, after, _context(finalizers=["ledger"]))

    assert _codes(applied) == ["VC013"]
...
"""
    _rewrite("VC013", before, after, _context(finalizers=["journal"]))
...
"""
    ...
    close()
"""
    _rewrite("VC013", before, after, _context(finalizers=["audited"]))
...
"""
    ...
    close()
"""
    _rewrite("VC013", before, after, _context(finalizers=["ledger"]))
...
"""
    ...
    file.close()
"""
    _rewrite("VC013", before, after, _context(finalizers=["handle"]))
...
"""
    ...
    assert _untouched("VC013", source, _context(finalizers=["ledger.register"])) == ()
...
"""
    applied = _rewrite("VC201", before, after, _context("test_x"))

    assert _codes(applied) == ["VC201"]
...
"""
    ...
    assert captured.out
"""
    _rewrite("VC201", before, after, _context("test_x"))
...
"""
    result = _apply("VC201", source, _context("test_x"))

    assert _codes(result.applied) == ["VC201", "VC201", "VC202"]
...
"""
    ...
    assert _codes(applied) == ["VC201", "VC201", "VC202"]
...
"""
    assert _untouched("VC201", source, _context("test_x")) == ()


def test_capsys_is_read_after_the_wiring_has_renamed_the_parameter() -> None:
    before = """def test_x(capture: Annotated[Any, Depends(voci.capture)]):
    assert capture.readouterr().out == ""
"""
    after = """def test_x(capture: Annotated[Any, Depends(voci.capture)]):
    assert capture.out == ""
"""
    _rewrite("VC201", before, after, _context("test_x"))


# --- VC204 caplog attributes ------------------------------------------------------------------
...
"""
    applied = _rewrite("VC204", before, after, _context("test_x"))

    assert _codes(applied) == ["VC204", "VC204"]


def test_caplog_text_and_record_tuples_move_to_log_records_under_vc206() -> None:
    before = """def test_x(caplog):
    assert caplog.text == ""
...
"""
    ...
    assert _codes(applied) == ["VC206", "VC206"]


def test_caplog_clear_call_moves_to_log_records_under_vc206() -> None:
    before = """def test_x(caplog):
    caplog.clear()
"""
    after = """def test_x(caplog):
...
"""
    assert _untouched("VC204", source, _context("test_x")) == ()
...
def test_x(caplog):
    with log_records.set_level(logging.INFO):
        ...
        assert caplog.messages == ["hello"]
"""
    applied = _rewrite("VC205", before, after, _context("test_x"))

    assert _codes(applied) == ["VC205"]
...
"""
    ...
    with log_records.set_level(logging.INFO, logger="app.db"):
        run()
"""
    _rewrite("VC205", before, after, _context("test_x"))
...
def test_a_conditional_set_level_is_refused() -> None:
    ...
    run()
"""
    assert _untouched("VC205", source, _context("test_x")) == ()
...
"""
    ...
    assert _codes(applied) == ["VC205", "VC205"]
...
def test_a_set_level_with_nothing_after_it_is_refused() -> None:
    ...
    caplog.set_level(logging.INFO)
"""
    applied = _untouched("VC205", source, _context("test_x"))
...
def test_a_raises_block_becomes_voci_raises() -> None:
    before = """import pytest
...
def test_x():
    with voci.raises(ValueError, match="nope") as info:
        boom()
    assert info.value
"""
    applied = _rewrite("VC209", before, after, _context("test_x"))

    assert _codes(applied) == ["VC209"]


def test_a_raises_called_with_a_func_becomes_voci_raises() -> None:
...
def test_x():
    voci.raises(ValueError, boom, 1)
"""
    applied = _rewrite("VC209", before, after, _context("test_x"))

    assert _codes(applied) == ["VC210"]
...
def test_x():
    voci.raises(ValueError, boom, 1, kind="bad")
"""
    applied = _rewrite("VC209", before, after, _context("test_x"))
...
def test_a_raises_called_with_match_is_refused() -> None:
    """pytest's callable form forwards `match=` to `func` as one of its `**kwargs`; voci's
...
"""
    applied = _untouched("VC209", source, _context("test_x"))
...
def test_a_raises_neither_entered_nor_called_is_refused() -> None:
    ...
    stays VC210: there is no `with` and no `func` for a rewrite to key off of."""
    source = """import pytest
...
def test_x():
    with voci.raises(ValueError):
        boom()
"""
    _rewrite("VC209", before, after, _context("test_x"))


# --- VC212 pytest.approx ---------------------------------------------------------------------


def test_approx_over_a_scalar_becomes_voci_approx() -> None:
    before = """import pytest
...
def test_x():
    assert value == voci.approx(0.3, rel=1e-6)
"""
    applied = _rewrite("VC212", before, after, _context("test_x"))

    assert _codes(applied) == ["VC212"]
...
def test_x():
    assert value == voci.approx(0.3, rel=1e-6)
"""
    _rewrite("VC212", before, after, _context("test_x"))


def test_approx_over_a_list_becomes_voci_approx() -> None:
    before = """import pytest
...
def test_x():
    assert values == voci.approx([0.1, 0.2])
"""
    applied = _rewrite("VC212", before, after, _context("test_x"))

    assert _codes(applied) == ["VC213"]


def test_approx_over_a_tuple_becomes_voci_approx() -> None:
    before = """import pytest
...
def test_x():
    assert values == voci.approx((0.1, 0.2), rel=1e-6)
"""
    applied = _rewrite("VC212", before, after, _context("test_x"))
...
def test_approx_over_a_dict_becomes_voci_approx() -> None:
    before = """import pytest
...
def test_x():
    assert values == voci.approx({"a": 0.1, "b": 0.2})
"""
    applied = _rewrite("VC212", before, after, _context("test_x"))
...
def test_approx_over_a_list_comprehension_becomes_voci_approx() -> None:
    before = """import pytest
...
def test_x():
    assert values == voci.approx([x for x in [0.1, 0.2]])
"""
    applied = _rewrite("VC212", before, after, _context("test_x"))
...
def test_x():
    assert values == pytest.approx({0.1, 0.2})
"""
    applied = _untouched("VC212", source, _context("test_x"))

    assert _codes(applied) == ["VC221"]
...
async def test_x(value, capsys, caplog):
    """Its own docstring."""
    with log_records.set_level(logging.INFO):
        with voci.raises(ValueError):  # loud
            assert capture.out == voci.approx(0.1)
        assert log_records.messages == []
'''
    )
...
# --- imperative skip/fail (bodies.VC214/VC223) -------------------------------------------------
...
"""
    applied = _rewrite("VC214", before, after, _context("test_x"))

    assert _codes(applied) == ["VC214"]
...
def test_skips_reason_keyword_becomes_positional() -> None:
    """`voci.Skipped` is a bare `BaseException`: raising one with `reason=` would raise
...
def test_x():
    raise voci.Skipped("no backend")
"""
    _rewrite("VC214", before, after, _context("test_x"))
...
def test_skip_with_allow_module_level_is_left_as_it_is() -> None:
    """No voci equivalent for pytest's import-time skip form."""
    source = """import pytest
...
"""
    applied = _untouched("VC214", source, _context("test_x"))
...
def test_xfail_called_as_a_statement_is_left_as_it_is() -> None:
    """There is no runtime target for it: `@voci.xfail(...)`'s condition is decided once, at
...
"""
    ...
    assert _codes(applied) == ["VC223"]


# --- VC034 relative imports --------------------------------------------------------------------
...
def test_x():
    assert STAMP
"""
    applied = _rewrite("VC034", before, after, _context("test_x"))

    assert _codes(applied) == ["VC034"]
...
def test_an_import_from_the_package_itself_names_the_package() -> None:
    ...
    assert _codes(_rewrite("VC034", before, after, _context())) == ["VC034"]
...
def test_an_import_reaching_above_the_root_is_left_as_it_was() -> None:
    ...
    assert _untouched("VC034", source, _context()) == ()
...
def test_a_level_import_matching_a_two_deep_file_still_resolves() -> None:
    ...
    assert _codes(_rewrite("VC034", before, after, context)) == ["VC034"]
...
def test_a_level_import_one_past_a_two_deep_file_is_left_as_it_was() -> None:
    ...
    assert _untouched("VC034", source, context) == ()
