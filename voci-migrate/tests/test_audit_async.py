...
def test_the_cases_a_backend_parametrization_puts_on_another_loop_are_blocked() -> None:
    ...
    findings = _by_code(result, "VC324")
    assert findings
...
def test_the_usefixtures_mark_the_plugin_hangs_on_a_test_is_not_the_suites() -> None:
    ...
    # migration deletes anyio rather than writing a `voci.use(...)` for what it wired.
    result = _audit(_with_anyio("asyncio"))
...
def test_a_fixture_the_runner_replaces_is_not_reported_as_one_with_no_voci_path() -> None:
    # anyio's whole job is running the test, which voci does itself, so `anyio_backend` is not a
    # dependency anybody has to write into the suite.
    result = _audit(_with_anyio("asyncio"))

    assert BACKEND_KEY not in {
        finding.detail.get("fixture") for finding in _by_code(result, "VC030")
    }
