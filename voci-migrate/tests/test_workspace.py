"""Tests for voci_migrate.workspace: the relocation branch, its rebase and export, and the
...
from voci_migrate import workspace
...
def test_snapshot_baseline_copies_everything_but_tool_state(tmp_path: Path) -> None:
    ...
    assert not (tree / ".voci-migrate").exists()
