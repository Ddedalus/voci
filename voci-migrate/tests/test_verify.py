"""Tests for voci_migrate.verify: reading each runner back, comparing the two, and the
...
from voci_migrate import cli, outcomes, verify
from voci_migrate.verify import report as verify_report
from voci_migrate.verify.runners import Run
...
def test_passes(value: int = voci.Depends(value)):
    assert value == 3


@voci.parametrize("case", ["a", "b b"])
def test_cases(case):
    assert case


@voci.skip(reason="not yet")
def test_skipped():
    raise AssertionError


def test_runtime_skip():
    raise voci.Skipped("nope")


@voci.xfail(reason="known")
def test_xfails():
    raise AssertionError
...
def test_two_runs_that_agree_have_nothing_to_review() -> None:
    ...
    after = run("voci", {"t.py::a": "passed", "t.py::b": "skipped"})
...
def test_each_kind_of_disagreement_is_named_and_ordered_worst_first() -> None:
    ...
    after = run("voci", {"t.py::moved": "failed", "t.py::new": "passed"})
...
def test_a_run_that_could_not_collect_is_not_a_clean_verification() -> None:
    ...
    after = run("voci", {"t.py::a": "passed"}, collection_errors=("u.py",))
...
def _voci_report(**overrides: object) -> dict:
    base: dict = {
        "report_version": 1,
        "runner": "voci",
        "exit_status": 0,
...
def test_a_voci_report_is_read_back_test_for_test() -> None:
    run = verify.voci_record(_voci_report(), tree=Path("."))
...
def test_a_voci_reports_collection_errors_carry_through() -> None:
    run = verify.voci_record(
        _voci_report(collection_errors=["tests/test_a.py"], tests=[]), tree=Path(".")
    )
...
def test_a_voci_report_from_another_version_of_the_tool_is_refused(tmp_path: Path) -> None:
    report = tmp_path / "voci-report.json"
    report.write_text(json.dumps(_voci_report(report_version=99)), encoding="utf-8")

    with pytest.raises(verify.RunnerError, match="report version"):
        verify.load_voci_report(report)
...
def test_a_run_of_a_suite_neither_runner_collected_is_not_a_pass() -> None:
    ...
    verification = verify.compare(run("pytest", {}), run("voci", {}))
...
def test_the_report_lists_every_divergence_the_terminal_summary_elides() -> None:
    ...
    after = run("voci", {f"t.py::test_{index}": "failed" for index in range(20)})
