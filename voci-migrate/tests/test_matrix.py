"""Tests for voci_migrate.matrix: the support matrix's own invariants.
...
from voci_migrate.matrix import Area, Disposition
...
def test_mechanical_and_marker_rows_are_the_ones_conversion_produces_code_for() -> None:
    ...
    assert "VC001" in converts
    assert "VC214" in converts
    assert "VC223" not in converts


def test_a_code_the_matrix_does_not_define_is_refused_by_name() -> None:
    with pytest.raises(KeyError, match="VC999"):
        matrix.construct("VC999")
