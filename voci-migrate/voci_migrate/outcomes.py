"""The pytest plugin that dumps what every test actually did, as JSON:
    ...
    pytest -p voci_migrate.outcomes --outcomes-out pytest-outcomes.json
...
DEFAULT_OUT = os.path.join(".voci-migrate", "pytest-outcomes.json")
...
def pytest_addoption(parser):
    ...
    group.addoption(
        ...
        metavar="PATH",
        help=(
            f"Write the voci-migrate outcomes record here (default: {DEFAULT_OUT}; the "
...
def _outcome_of(report):
    ...
    if report.when == "call":
        if expected_failure:
            ...
            # one here, which is what voci reports for it too.
    ...
    if report.outcome == "failed":
        ...
        # rather than a failure — the same line voci draws.
        return "error"
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if reporter is not None:
        reporter.write_line(f"voci-migrate: wrote {len(_outcomes)} outcomes to {out}")
