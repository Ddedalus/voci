...
def load(path: str | Path) -> dict:
    ...
    try:
        ...
        raise DumpError(
            f"No ground-truth dump at {path}. Run `voci-migrate extract` against the suite, "
            "or run pytest with `-p voci_migrate.extractor --collect-only` in the environment "
...
def _check_version(dump: dict, source: str) -> None:
    ...
    if found is None:
        raise DumpError(
            f"{source} carries no `extractor_version`, so it did not come from voci-migrate's "
    ...
    raise DumpError(
        f"{source} was written by extractor version {found}, and this voci-migrate reads "
        f"version {EXTRACTOR_VERSION}. The dump is {direction} than the tool: re-run the "
        "extractor shipped with this voci-migrate against the suite."
    )
