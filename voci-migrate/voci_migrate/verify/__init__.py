...
This is the stage that answers that: pytest on the pre-migration tree, `voci --serial` on the
...
from voci_migrate.verify.runners import (
    DEFAULT_BASELINE,
    Run,
    RunnerError,
    load_record,
    ...
    *,
    ...
    if before_tree is None and (paths or pytest_args):
        raise RunnerError(
            ...
            "whatever the run that wrote it collected, and narrowing only voci would "
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        after_future = pool.submit(run_voci, after_tree, paths=paths, concurrency=concurrency)
