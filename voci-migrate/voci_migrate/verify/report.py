...
from voci_migrate.report.markdown import table
from voci_migrate.report.payload import write_json
from voci_migrate.verify import Divergence, Run, Verification
...
_HEADLINES = {
    "missing": "ran under pytest, never ran under voci",
    "unexpected": "ran under voci, never ran under pytest",
...
def markdown(verification: Verification) -> str:
    ...
    for kind, items in verification.by_kind().items():
        lines += [
            ...
            f"{_HEADLINES[kind]}.",
            "",
            table(("test", "pytest", "voci"), [_divergence_row(item) for item in items]),
            "",
        ]
    return "\n".join(lines).rstrip("\n") + "\n"
