...
pytest reports through `voci_migrate.outcomes`, which writes a JSON record; voci reports through
...
from voci_migrate.schema import CLEAN_EXIT_STATUSES
...
DEFAULT_BASELINE = os.path.join(".voci-migrate", "pytest-outcomes.json")
OUTCOMES_VERSION = 1

# Kept in step with `voci._report.json_report.REPORT_VERSION`, which cannot be imported from
# either: `verify` runs voci as a subprocess in a tree of its own, possibly a different
# environment from the one voci-migrate itself is installed in.
VOCI_REPORT_VERSION = 2
...
def run_voci(tree: Path, *, paths: list[str], concurrency: int) -> Run:
    """Run the converted suite under voci at `concurrency`, reading `--report-json` back.
    ...
    """
    executable = _voci_executable()
    if executable is None:
        raise RunnerError(
            "no `voci` command on PATH. `verify` runs both runners, so voci has to be "
            "installed in this environment alongside pytest."
        )
    with tempfile.TemporaryDirectory() as scratch:
        report_path = Path(scratch) / "voci-report.json"
        command = [
            executable,
            *paths,
            "--concurrency",
        ...
        loaded = load_voci_report(report_path)
    return voci_record(loaded, tree=tree, duration=duration, command=command)


def load_record(path: Path) -> dict:
    """The JSON `voci_migrate.outcomes` wrote at `path`, refused unless this tool understands
    its shape."""
    try:
        ...
        raise RunnerError(
            ...
            "`voci-migrate verify --record --before <tree>`, or point `--before` at a tree "
...
def load_voci_report(path: Path) -> dict:
    ...
    if not isinstance(loaded, dict) or loaded.get("report_version") != VOCI_REPORT_VERSION:
        ...
        raise RunnerError(
            ...
            f"{VOCI_REPORT_VERSION}. Is the `voci` on PATH the version `verify` expects?"
        )
    return loaded
...
def voci_record(
    ...
    return Run(
        runner=str(loaded.get("runner", "voci")),
        tree=tree,
...
def _voci_executable() -> str | None:
    """The `voci` command to run, preferring the one installed beside this interpreter.

    `verify` assumes one environment holding both runners, and the environment voci-migrate is
    running in is the one that assumption is about — searching PATH first would find whatever
    voci happens to be on it, which under a `uv run`-style launcher is often nothing at all.
    """
    beside = Path(sys.executable).parent
    return shutil.which("voci", path=os.pathsep.join([str(beside), os.environ.get("PATH", "")]))
