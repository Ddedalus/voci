...
from voci_migrate.convert import DEFERRED, Conversion
...
def _declared(conversion: Conversion) -> list[str]:
    """Where each `voci.use(...)` goes, since a declaration reaches tests that never name it."""
    built = conversion.plan
    ...
    for declaration in built.declarations[:_NAMED]:
        ...
        lines.append(f"  {declaration.container}: voci.use({names})")
...
def _settings(conversion: Conversion) -> list[str]:
    ...
    if settings.conflict:
        return [f"[tool.voci]: left alone — {settings.conflict}"]
    ...
    # `xfail_strict` is written into each `@voci.xfail`, both of which the sections above show.
    if settings.dropped:
