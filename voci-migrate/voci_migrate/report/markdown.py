...
from voci_migrate.audit.findings import Audit, Finding, Unannotated, Unclassified, ordered
...
def _opening(audit: Audit) -> list[str]:
    ...
    return [f"# Migrating {suite.rootpath} to voci", read]
...
def _caveats(audit: Audit) -> list[str]:
    ...
    blocks = [
        ...
        "These convert and run. Each leaves a `VOCI-TODO` marker in the converted source at the "
        "point where voci's behaviour differs, so the caveat travels with the code and can be "
        "grepped for.",
    ]
    for code, findings in groups:
        blocks += _group(code, findings)
...
def _configuration(audit: Audit) -> list[str]:
    ...
    blocks = [
        ...
        "What the suite's settings and installed plugins amount to under voci, worst first. "
        "Unknown `[tool.voci]` keys are a hard error, so nothing here is written speculatively.",
    ]
    for code, findings in groups:
...
def _group(code: str, findings: Sequence[Finding], *, label_disposition: bool = False) -> list[str]:
    ...
    body += f". {row.note}"
    if row.marker:
        body += f" Converted source carries `VOCI-TODO[{row.marker}]`."
