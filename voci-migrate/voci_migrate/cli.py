"""The `voci-migrate` command line.

`extract` is a convenience wrapper: it runs pytest, as a subprocess, with the extractor plugin
loaded. Where that environment cannot be arranged, `voci_migrate/extractor.py` copied next to
...
from voci_migrate import schema, workspace
from voci_migrate.audit import DEFAULT_BUDGET, sources_of
from voci_migrate.model import GroundTruth
from voci_migrate.verify.runners import DEFAULT_BASELINE
...
DEFAULT_OUT = os.path.join(".voci-migrate", "ground-truth.json")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="voci-migrate", description="Migrate a pytest suite to voci."
    )
    ...
    convert = commands.add_parser(
        "convert",
        help="rewrite the suite as a voci one",
        description=(
            "Translate the suite's wiring, marks, bodies and configuration into their voci "
            ...
            "the tree and records its pytest outcomes into .voci-migrate/baseline/ -- the last "
    ...
    convert.add_argument(
        ...
        default="",
        metavar="CODES",
        help="comma-separated support-matrix codes whose rewrite rule to skip (e.g. VC101,VC204)",
    )
    convert.add_argument(
        "--budget",
        type=int,
    ...
    verify = commands.add_parser(
        ...
        description=(
            "Run pytest on the pre-migration tree and voci on the converted one, then compare "
    ...
    verify.add_argument(
        ...
        default=".",
        metavar="DIR",
        help="the converted tree, run under voci (default: the current directory)",
    )
    verify.add_argument(
        "--baseline",
    ...
    verify.add_argument(
        ...
        metavar="N",
        help=(
            "what concurrency to run voci at (default: 1, serial — the run that separates a "
...
def _extract(args: argparse.Namespace, passthrough: list[str]) -> int:
    if importlib.util.find_spec("pytest") is None:
        print(
            "voci-migrate: `extract` collects the suite with pytest, and this environment has "
            "none. Install pytest here, or copy voci_migrate/extractor.py into the environment "
    ...
    if completed.returncode not in schema.CLEAN_EXIT_STATUSES:
        print(
            f"voci-migrate: pytest exited {completed.returncode} without collecting this suite, "
    ...
    try:
        ...
        print(f"voci-migrate: {exc}", file=sys.stderr)
        return 1
    return 0


def _audit(args: argparse.Namespace, passthrough: list[str]) -> int:
    from voci_migrate import audit, report
...
def _convert(args: argparse.Namespace, passthrough: list[str]) -> int:
    from voci_migrate import audit, convert, report
...
def _record_baseline(root: Path, passthrough: list[str]) -> str | None:
    """Snapshot `root` and record its pytest outcomes into `.voci-migrate/baseline/`, the last
    ...
    if importlib.util.find_spec("pytest") is None:
        print(
            "voci-migrate: `convert --write` records a pytest baseline before it overwrites "
            f"{root}, and this environment has no pytest. Install pytest here, or record the "
            "baseline yourself in the suite's own environment with `voci_migrate/outcomes.py` "
            "copied in and its output passed back as --baseline, before converting.",
            file=sys.stderr,
        )
        return None

    from voci_migrate import verify
...
def _verify(args: argparse.Namespace, passthrough: list[str]) -> int:
    from voci_migrate import verify
    from voci_migrate.verify import report as verify_report
    ...
    if args.record:
        if before_tree is None:
            print(
                "voci-migrate: `--record` records the pytest side, so it needs `--before` "
...
def _load(args: argparse.Namespace) -> tuple[GroundTruth, Path] | None:
    ...
    if root is None:
        print(
            f"voci-migrate: none of the {len(sources_of(ground_truth))} source files the "
