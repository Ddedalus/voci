"""The tests voci runs by itself, and the decorator that says so.

`unittest.mock` installs a patch by writing to a module or a class, which every test running at
the same time sees. A patch applied as a decorator is on the function object, so voci finds it at
collection and schedules that test alone without being told. A patch entered inside a body is not
there to be found until the body runs, so the test carries `@voci.solo` and nothing else runs
while it holds.

Idempotent by reading the decorators a function already carries rather than by tracking what an
earlier run wrote, so a hand-written `@voci.solo` counts the same as a generated one.
"""
...
def apply(module: cst.Module, qualnames: Collection[str]) -> cst.Module:
    """`module` with `@voci.solo` above each function `qualnames` names, by qualified name."""
    if not qualnames:
        return module
...
class _Solo(cst.CSTTransformer):
    """Attaches `@voci.solo` to the functions named, tracking qualified names as it goes."""
    ...
    def leave_FunctionDef(
        ...
        if qualname not in self._qualnames or _already_solo(updated_node):
            return updated_node
        # Outermost, where a reader meets it first: voci reads a test's marks through whatever
...
def _is_solo(expression: cst.BaseExpression) -> bool:
    match expression:
        case cst.Attribute(value=cst.Name(value="voci"), attr=cst.Name(value="solo")):
            return True
        case _:
            return False
...
def _decorator() -> cst.Decorator:
    return cst.Decorator(decorator=cst.Attribute(value=cst.Name(VOCI), attr=cst.Name(SOLO)))
