...
pytest let a class own fixtures, visible to its methods and to nothing else. A voci test class is
...
from voci_migrate.convert.plan import FixtureWork
...
def _drained(
    ...
    if not kept:
        # A class that wrote nothing but fixtures still names a group voci collects tests under,
...
class _ThroughClass(cst.CSTTransformer):
    ...
    every other `self` under `VC033` before a fixture reaches this, so a `self` still standing is
