"""VC024: the cases a hook built while pytest collected, written out as the parametrize that
lists them.

`pytest_generate_tests` runs during collection, so the cases it produced are in the dump and the
hook itself has nothing to become — voci reads a test's cases off the test. This rule writes the
frozen list the plan read out of the dump, one decorator per axis, outermost first: voci reads
...
decorator itself: a test already carrying a `@voci.parametrize` over the same argnames is one an
...
class _Generated(RuleTransformer):
    """VC024: a `@voci.parametrize` per axis a hook produced, on each test it produced them for."""
    ...
    def leave_FunctionDef(
        ...
        for axis in missing:
            self.record(
                ...
                f"{len(axis.values)} case(s) are written out as `@voci.parametrize`.",
                code="VC024" if axis.ids is not None else "VC114",
            )
...
def _written(node: cst.FunctionDef) -> frozenset[str]:
    """The axes this function already carries a `@voci.parametrize` for, by their argnames."""
    found: set[str] = set()
    for decorator in node.decorators:
        call = decorator.decorator
        if not isinstance(call, cst.Call) or dotted(call.func) != "voci.parametrize":
            continue
...
def _decorator(axis: Generated) -> cst.Decorator:
    ...
    return cst.Decorator(decorator=cst.Call(func=voci("parametrize"), args=args))
