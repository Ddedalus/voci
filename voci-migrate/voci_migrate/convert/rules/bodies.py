...
rewritten only in the shapes voci has: a `raises` entered by a `with` or called with the
...
of `voci.Skipped`/`voci.Failed` -- the runtime counterparts of the `skip`/`xfail` marks, for a
body that decides mid-run rather than at collection. `pytest.xfail` the same way stays refused:
there is no runtime target for an expectation `@voci.xfail(...)` only ever decides once, ahead of
...
from voci_migrate.audit.sources import APPROX_KINDS, approx_nested, imperative_reason_args
...
# The attributes `voci.log_records` carries under the same names `caplog` did, mapped to the
# code each rename files under: `records`/`messages` are this class's own default (VC204);
# `text`/`record_tuples` are VC206's promoted rename onto the same object.
...
# The shapes of `pytest.approx`'s first argument that convert under VC213 rather than the default
# VC212 (a scalar). A literal's own elements are what `approx_nested` walks; a comprehension's
...
# `voci.raises` refuses to catch a cancellation, which is how a timeout stops a runaway test, so
...
class _Capture(_BodyPass):
    """VC201: `capsys.readouterr()`, whose `.out` and `.err` are live on `voci.capture`.
    ...
    def leave_Call(self, original_node: cst.Call, updated_node: cst.Call) -> cst.BaseExpression:
        ...
        self.record(
            ...
            "nor unpacked, and `voci.capture` is not the snapshot `readouterr()` returns."
        )
        return updated_node
    ...
    def leave_FunctionDef(
        ...
        if reads > 1:
            self.record(
                f"`readouterr()` is read {reads} times here, and `voci.capture` is cumulative: "
...
class _LogRecords(_BodyPass):
    """VC204: `caplog.records` and `caplog.messages`, the same two names on `voci.log_records`.
    VC206: `caplog.text`, `caplog.record_tuples` and `caplog.clear()`, renamed the same way but
    filed under their own code since VC204's row is specifically about `records`/`messages`.
    """
    ...
    def leave_Call(self, original_node: cst.Call, updated_node: cst.Call) -> cst.BaseExpression:
        ...
        if held is None or held == LOG_RECORDS:
            return updated_node
        self.record(f"`{render(original_node)}` becomes `{LOG_RECORDS}.clear()`.", code="VC206")
        return updated_node.with_changes(func=func.with_changes(value=cst.Name(LOG_RECORDS)))


class _SetLevel(_BodyPass):
    """VC205: `caplog.set_level(...)`, as a `with` block over the rest of the test.
...
class _Raises(_BodyPass):
    """VC209: `pytest.raises` where a `with` enters it, and VC210: the callable form
    `pytest.raises(E, func, *args, **kwargs)` — both are shapes voci's `raises` has too, so both
    rewrite to `voci.raises` unchanged but for the name. A `pytest.raises(E)` that is neither —
    entered by nothing, called with no second positional argument — is a raises object being
    stashed for later, which still reports VC210: there is no `with` and no `func` for a rewrite
    to key off of. A callable form passing `match=` is left alone too: pytest forwards it to
    `func` there, but `voci.raises` always intercepts it, so the two forms disagree on what the
    ...
    def _with_entered(self, original_node: cst.Call, updated_node: cst.Call) -> cst.BaseExpression:
        ...
        if starred(original_node) or len(given) != 1:
            self.record(
                f"`{render(original_node)}` is left as it is: `voci.raises` takes the expected "
        ...
        if uncatchable is not None:
            self.record(
                f"`{render(original_node)}` is left as it is: `voci.raises` refuses "
                f"`{uncatchable}`, since cancellation is how voci enforces a timeout.",
                code="VC211",
            )
            return updated_node
        ...
        if unknown:
            self.record(
                f"`{render(original_node)}` is left as it is: `voci.raises` has no "
                f"`{'`, `'.join(sorted(unknown))}`.",
            )
            return updated_node
        self.record(f"`{render(original_node)}` becomes `voci.raises`.")
        return updated_node.with_changes(func=voci("raises"))
    ...
    def _callable(self, original_node: cst.Call, updated_node: cst.Call) -> cst.BaseExpression:
        ...
        if len(given) < 2:
            self.record(
                f"`{render(original_node)}` is left as it is: `voci.raises` is a context "
        ...
        if "match" in keywords(original_node) or double_starred(original_node):
            self.record(
                ...
                "in this form, but `voci.raises` always intercepts it to match the exception, "
        ...
        if uncatchable is not None:
            ...
            return updated_node
        self.record(f"`{render(original_node)}` becomes `voci.raises`.", code="VC210")
        return updated_node.with_changes(func=voci("raises"))

    def _uncatchable(self, expected: cst.BaseExpression) -> str | None:
        """Which of the types `voci.raises` refuses `expected` names, if it names one."""
        candidates = [expected]
...
class _Approx(_BodyPass):
    """VC212: `pytest.approx` over a scalar, which is the default this class's own `CODE` files
    under. VC213: `pytest.approx` over a list, a tuple, or a dict — an explicit `code=` override
    ...
    one level inside a list/tuple/dict literal is one such refusal, since `voci.approx` only
    walks one level. VC221: a set, a set comprehension, a generator expression, or a numpy array,
    ...
    def leave_Call(self, original_node: cst.Call, updated_node: cst.Call) -> cst.BaseExpression:
        ...
        if starred(original_node) or not given:
            self.record(
                f"`{render(original_node)}` is left as it is: `voci.approx` takes the expected "
                "value itself.",
                code=code,
            )
            return updated_node
        ...
        if kind is not None:
            self.record(
                ...
                "`voci.approx` has no position to compare it by.",
                code="VC221",
            )
            return updated_node
        ...
        if nested is not None:
            self.record(
                ...
                "inside it, and `voci.approx` has no position to compare a nested container by.",
                code=code,
            )
            return updated_node
        ...
        if unknown or len(given) > 1 + len(_APPROX_POSITIONAL):
            self.record(
                f"`{render(original_node)}` is left as it is: `voci.approx` takes `rel`, `abs` "
                "and `nan_ok`.",
                code=code,
            )
            return updated_node
        ...
        self.record(f"`{render(original_node)}` becomes `voci.approx`.", code=code)
        return updated_node.with_changes(func=voci("approx"), args=args)

    def _code(self, expected: cst.BaseExpression) -> str | None:
        """`"VC213"` where `expected` is list/tuple/dict shaped, else `None` (the default
        `VC212`, a scalar)."""
        return "VC213" if isinstance(expected, _APPROX_CONTAINER_SHAPES) else None

    def _kind(self, expected: cst.BaseExpression) -> str | None:
        """What `expected` is, where it is something `voci.approx` does not compare."""
...
class _Imperative(_BodyPass):
    """VC214: `pytest.skip(reason)`/`pytest.fail(reason)`, called as a bare statement, becoming
    `raise voci.Skipped(reason)`/`raise voci.Failed(reason)` -- the two are `BaseException`s a
    body raises directly, not decorators like `voci.skip`/`voci.xfail`, whose own row this
    would otherwise collide with (calling a decorator factory in a body builds a decorator and
    discards it, turning a conditional skip into a silent pass). VC223: `pytest.xfail(...)` the
    same way, which stays refused -- there is no runtime target for it, since `@voci.xfail`'s
    ...
    a keyword, or both of fail's two spellings) is left as it is -- none of those has a voci
    ...
    def leave_Expr(self, original_node: cst.Expr, updated_node: cst.Expr) -> cst.BaseSmallStatement:
        ...
        if name == "pytest.xfail":
            self.record(
                ...
                "`@voci.xfail(...)`'s condition is decided once at collection, before the test "
                "has run.",
                code="VC223",
            )
            return updated_node
        # The eligibility check -- and what counts as *the* reason -- lives in `audit.sources`,
        # which the VC214 scan reads the very same call for ahead of any rewrite running; a
        ...
        if reason_args is None:
            self.record(
                f"`{render(call)}` is left as it is: `voci.{self._TARGETS[name]}` takes only "
                "the reason, positionally."
            )
            return updated_node
        target = self._TARGETS[name]
        self.record(f"`{render(call)}` becomes `raise voci.{target}(...)`.")
        args = [argument(reason) for reason in reason_args]
        return cst.Raise(exc=cst.Call(func=voci(target), args=args))


class _GetFixtureValue(_BodyPass):
    """VC011: `request.getfixturevalue("name")`, which is a parameter the signature grows.
...
class _Finalizers(_BodyPass):
    """VC013: `request.addfinalizer(fn)`, which is what a `yield` fixture's teardown says.
