...
voci imports test modules under synthetic `voci_tests.*` names and takes them back out of
...
from voci_migrate.convert.rules import Rule, RuleTransformer, rule

__all__ = ["RULES"]


class _Relative(RuleTransformer):
    """VC034: `from .sibling import name` as the absolute import of the same module."""
    ...
    def leave_ImportFrom(
        ...
        if absolute is None:
            return updated_node
        self.record(
            f"`{_spelled(updated_node)}` is relative, and voci imports this module under a "
