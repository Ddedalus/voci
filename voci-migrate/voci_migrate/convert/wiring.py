...
voci reads an injection out of the parameter's annotation metadata, so nothing a signature grows
...
without a default cannot follow one that has it — and voci binds every argument by keyword, so
...
named only by a `TYPE_CHECKING` import would be a `NameError` at import time. voci reads the
annotation under either spelling by parsing its source text for the `Depends()` marker, evaluating
that marker and nothing else, as `voci/_di/fixtures.py` says at the top.
"""
...
from voci_migrate.convert.annotate import TypeImport, statements
from voci_migrate.convert.plan import FileWork, FixtureWork, Injection, TestWork
from voci_migrate.inference import bindings, free
from voci_migrate.model import REQUEST

# The case argument voci binds a parametrized fixture's value to; there is no `request`.
PARAM = "param"
...
_STRANDED: Mapping[str, str] = {"capsys": "VC202"}
...
class Result:
    ...
    says why — a `request` a rewrite could not eliminate, or an injection in a position voci
    cannot bind.
    """
...
def apply(
    ...
    `voci.` name into this module, which is what makes the import of `voci` needed even where
...
def tidy(module: cst.Module) -> cst.Module:
    ...
    when the signatures were rewritten is a voci tag by the time the last rule has been through,
...
class _Wiring(VisitorBasedCodemodCommand):
    ...
    def leave_FunctionDef(
        ...
        if stranded is not None:
            ...
            # rewrite every use of `capsys` and `caplog` they have a voci spelling for, so a use
    ...
    def leave_Module(self, original_node: cst.Module, updated_node: cst.Module) -> cst.Module:
        ...
        # anyone asked for.
        if self._wrote:
            AddImportsVisitor.add_needed_import(self.context, "voci")
        if self._injected:
            AddImportsVisitor.add_needed_import(self.context, "voci", "Depends")
    ...
    def _fixture(
        ...
        if not _request_is_only_param(original):
            self.refused.add((work.symbol, "VC015"))
            return original
        decorators = [
    ...
    def _signature(
        ...
        if any(param.name.value in by_name for param in node.params.posonly_params):
            # voci binds every argument by keyword, so a positional-only parameter can never be
            # injected. Rewriting the signature would change what the test can be called with.
            self.refused.add((name, "VC036"))
            return node
        self._wrote = True
        ...
        # positional parameter without one has nowhere valid to go — voci binds by keyword, so a
...
def _inject(param: cst.Param, injection: Injection | None, claims: _Claims) -> cst.Param:
    ...
    # a `voci.Capture`, so the `CaptureFixture[str]` the source wrote is no longer true of it.
...
def _stranded(node: cst.FunctionDef, injections: Sequence[Injection]) -> str | None:
    ...
    A builtin whose voci counterpart is a different object is renamed with the parameter, and
    every use the rules could translate is already translated by the time this runs. What is left
    is a use with no voci spelling, and the row that says so depends on which one it is.
    """
    for injection in injections:
        ...
        if injection.was == CAPLOG:
            return "VC205" if _mentions(node.body, SET_LEVEL) else "VC222"
        return _STRANDED.get(injection.was, "VC220")
    return None
...
def _is_fixture_decorator(expression: cst.BaseExpression) -> bool:
    ...
    planning pass that named the fixture agree about which definitions are fixtures. `voci.fixture`
    ...
    match target:
        case cst.Attribute(value=cst.Name(value="voci"), attr=cst.Name(value="fixture")):
            return False
...
def _fixture_call(expression: cst.BaseExpression, work: FixtureWork) -> cst.BaseExpression:
    """`@voci.fixture(...)`, carrying over the arguments that still mean something.

    `scope` is written only where it is not voci's default, `params` and `ids` come across
    ...
    if work.carried is not None:
        ...
        # with it — pytest tolerates an `ids=` with no `params=` beside it, and voci does not.
    ...
    return cst.Call(
        func=cst.Attribute(value=cst.Name("voci"), attr=cst.Name("fixture")),
...
class _RequestParam(cst.CSTTransformer):
    """Rewrites `request.param` to the `param` argument voci binds the case to."""
