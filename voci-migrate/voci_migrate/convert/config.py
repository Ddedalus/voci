"""The suite's pytest configuration as a `[tool.voci]` table.

`[tool.voci]` rejects unknown keys, so a setting is either carried under an exact voci spelling
or reported as dropped; nothing is written on the chance that voci might understand it. What
...
A `[tool.voci]` table that is already there is read, compared and left alone, so the one file the
...
from voci_migrate.audit.config import written_settings
from voci_migrate.convert.edits import Edit
from voci_migrate.model import GroundTruth

__all__ = ["CARRIED", "PYPROJECT", "Translation", "edit", "translate"]

#: Where a suite's voci configuration goes, rootdir-relative.
PYPROJECT = "pyproject.toml"

HEADER = "[tool.voci]"

#: pytest setting -> `[tool.voci]` key, for the settings that mean the same thing on both sides:
#: support-matrix rows VC301, VC302, VC303, VC304 and VC307.
...
# `voci._config`'s own key order, which the table follows so two conversions read alike.
_ORDER = (
    "testpaths",
    "concurrency",
    "timeout",
...
# The type each carried voci key takes, as `voci._config` validates it.
...
_TABLE = re.compile(r"^\[tool\.voci(\.|\])", re.MULTILINE)
_KEY = re.compile(r"^([A-Za-z0-9_-]+)\s*=")
_SUBTABLE = re.compile(r"^\[tool\.voci\.([^\]]+)\]")


@dataclass(frozen=True, slots=True)
class Translation:
    """What one suite's pytest configuration becomes under voci.

    `settings` is voci key -> TOML value text in the order `table` writes them, and `carried` says
    which pytest setting each of them came from. `dropped` names the settings the suite wrote that
    voci has no key for, together with any whose value could not be rendered — a value is never
    guessed at. `conflict` says why an existing `[tool.voci]` was left as it is, and is `None` both
...
def translate(ground_truth: GroundTruth, *, root: Path) -> Translation:
    """The `[tool.voci]` table for the suite `ground_truth` describes, whose files are at `root`.
    ...
    for pytest_key in sorted(written & set(CARRIED), key=lambda key: _ORDER.index(CARRIED[key])):
        voci_key = CARRIED[pytest_key]
        value = _value(ground_truth, pytest_key, voci_key)
        if value is None:
            continue
        settings[voci_key] = value
        carried[pytest_key] = voci_key
...
def edit(translation: Translation, *, root: Path) -> Edit | None:
    ...
    `None` when the file already declares `[tool.voci]`, whether or not it declares the same
...
def _written(ground_truth: GroundTruth, root: Path) -> set[str]:
    ...
    wrote it or not, and turning that into a voci `ignore` key would hand the suite a setting it
    never had.
    """
...
def _value(ground_truth: GroundTruth, pytest_key: str, voci_key: str) -> str | None:
    """`pytest_key`'s value as the TOML text `voci_key` takes, or `None` if it has no such text.
    ...
    try:
        ...
        return None
    if voci_key not in _LISTS:
        return _number(value)
    return _list(_plain_names(value) if voci_key == "ignore" else value)


def _plain_names(value: object) -> object:
    """`norecursedirs` narrowed to the entries voci's `ignore` can honour.

    pytest matches `norecursedirs` as fnmatch patterns against each directory it walks into;
    voci's `ignore` is a set of directory names it compares exactly. A pattern carried across
...
def _list(value: object) -> str | None:
    ...
    an empty voci list is an instruction — collect nothing, ignore nothing — rather than silence.
    """
...
def _render(settings: Mapping[str, str]) -> str:
    ...
    The header goes in even when there is nothing under it. `[tool.voci]` is what marks the
...
def _existing(root: Path) -> str | None:
    """The `[tool.voci]` block already in `<root>/pyproject.toml`, if there is one."""
    text = _read(Path(root, PYPROJECT))
    return None if text is None else _table_of(text)


def _table_of(text: str) -> str | None:
    """The `[tool.voci]` block of `text`, sub-tables included, or `None` if it declares none.

    Ends at the next table header that is not voci's own, with the blank lines before it dropped,
...
def _conflict(existing: str | None, settings: Mapping[str, str], table: str) -> str | None:
    """Why the `[tool.voci]` table in the file is kept over the one this would write."""
    if existing is None or existing == table:
        return None
    theirs = ", ".join(key for line in existing.splitlines() if (key := _key_of(line)))
    return (
        f"{PYPROJECT} already configures voci, setting {theirs or 'nothing'}, where this "
        f"conversion would set {', '.join(settings) or 'nothing'}. Its table is left as it is."
    )


def _key_of(line: str) -> str | None:
    """What `line` sets, whether it sets a key or opens a sub-table like `[tool.voci.env]`."""
