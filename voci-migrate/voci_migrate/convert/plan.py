...
Refusal is the load-bearing half. A refused test keeps its pytest signature, which voci reports
...
from voci_migrate.audit import Audit, Finding, Site, sources_of
from voci_migrate.audit import wiring as audit_wiring
from voci_migrate.convert import declarations, layout, parametrize, specialize
from voci_migrate.convert.annotate import SAME_AS_PYTEST, Degraded, Resolver, TypeImport
from voci_migrate.convert.declarations import Declaration
from voci_migrate.convert.layout import Import, Layout
from voci_migrate.convert.parametrize import Carried, Decision
from voci_migrate.convert.rules import Context
from voci_migrate.convert.specialize import Specialization
from voci_migrate.model import REQUEST, Dependency, FixtureDef, GroundTruth, Item
...
_DYNAMIC = ("VC011", "VC013")
_REQUEST_SURVIVES = ("VC012", "VC014", "VC015", "VC016", "VC017")
_DECLINED = {"VC011": "VC028", "VC013": "VC014"}

# pytest's own fixtures with a voci counterpart, and the parameter name the translation binds.
# `capsys` and `caplog` are renamed because their voci counterparts are different objects with
# different methods, and a body reading `capsys.readouterr()` has to be rewritten anyway.
BUILTINS: Mapping[str, tuple[str, str]] = {
    "tmp_path": ("tmp_path", "voci.tmp_path"),
    "tmp_path_factory": ("tmp_path_factory", "voci.tmp_path_factory"),
    "tmpdir": ("tmpdir", "voci.tmpdir"),
    "tmpdir_factory": ("tmpdir_factory", "voci.tmpdir_factory"),
    "capsys": ("capture", "voci.capture"),
    "caplog": ("log_records", "voci.log_records"),
}

# The voci scope a pytest scope becomes. voci has four, and the two pytest has that it does not
# widen to the next one out, which is `VC003`.
...
class Injection:
    ...
    afterwards, which differs from `was` only where the voci counterpart is a different object,
    ...
    is true only of a built-in voci hands back as a different object — `capsys` stops being a
...
class FileWork:
    ...
    `fixtures.py` beside it. `marks` are the `VOCI-TODO` comments to attach, each keyed by the
    qualname it belongs above and empty for one about the module itself. `declares` are the
    fixtures this file names in a `voci.use(...)`, as the expressions naming them here, and a
...
class Plan:
    ...
    ones included: voci walks up from a test file and stops at the first directory that is not a
...
def refused(finding: Finding) -> bool:
    ...
    the construct converts and the report says which tests end up needing `@voci.solo` — refusing
...
def _declared_fixtures(sources: Mapping[str, str]) -> dict[str, dict[str, Declared]]:
    ...
    A container is a file, or a `file::Class` for the fixtures a test class writes: voci test
...
def _bodies(
    ...
    *,
    ...
    the parameter behind, and a parameter left behind is a fixture voci cannot construct. So a
    ...
    for finding in sorted(audit.findings, key=lambda finding: finding.site.sort_key):
        ...
        bodies[site] = Body(
            ...
            finalizers=body.finalizers or finding.code == "VC013",
        )
...
def _requested(
    ...
    writes an object for, or one of pytest's own that voci provides under another name.
    """
    if finding.code != "VC011":
        return ()
    name = str(finding.detail.get("requested", ""))
...
def _solo(
    ...
    the source pytest ran, and a mark on it would say nothing about how voci schedules it.
    """
    alone = {
        nodeid for finding in audit.findings if finding.code == "VC218" for nodeid in finding.tests
    }
...
def _undeclarable(overrides: Sequence[audit_wiring.Override]) -> set[str]:
    ...
    A `voci.use(...)` names one object for a directory, so an autouse fixture that an override
...
def _unnameable(
    ...
    Either it is already refused, or it is a fixture from outside the suite with no voci
    counterpart — a plugin's, or one of pytest's own that voci does not provide.
    """
...
def _without_request(
    ...
    *,
    ...
    nothing left to bind once they are rewritten, and voci has nothing to bind it to.
    """
    ...
    *,
    ...
    for name in names:
        ...
        if builtin is not None:
            ...
            # `annotate.BUILTIN_TYPES` rather than from inference — and for all but the ones voci
...
def _marks(
    ...
    """The `VOCI-TODO` comments to attach, by file, as (qualname, code) pairs."""
...
def _param_of(ground_truth: GroundTruth, key: str, name: str) -> str:
    """The parameter a requested name becomes, which is its own unless voci renames the fixture."""
    fixture = ground_truth.fixture_defs.get(key)
