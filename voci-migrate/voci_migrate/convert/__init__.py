"""The conversion: a pytest suite rewritten as a voci one, deterministically.

`convert` is the one stage with no verification oracle behind it — the suite is green under pytest
before it runs and green under voci only after `verify` says so — which is why it is mechanical,
...
the wiring swap, which owns signatures, fixture decorators and imports; then the `voci.use(...)`
declarations, which name what the swap has just imported; then `@voci.solo` on the tests that take
...
from voci_migrate.convert import (
    config,
    declarations,
    layout,
    lift,
...
from voci_migrate.convert.edits import Edit, EditSet
from voci_migrate.convert.plan import DEFERRED, FileWork, Plan
