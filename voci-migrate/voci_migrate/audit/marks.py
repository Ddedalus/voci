...
file — and voci has no such inheritance: a mark ends up on the test itself. So the questions here
...
from voci_migrate.model import GroundTruth, Item, literal

# Marks voci records once per test, so two of them is a collection error rather than a contest.
...
def _usefixtures_findings(ground_truth: GroundTruth, reach: Reach) -> Iterator[Finding]:
    # Keyed by module and fixture name: `voci.use(...)` is declared per module, so whether a
    ...
    for (path, name), tests in sorted(covered.items()):
        ...
        yield Finding(
            code="VC010" if widens else "VC009",
            message=(
