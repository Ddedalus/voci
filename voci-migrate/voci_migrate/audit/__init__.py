...
from voci_migrate.audit import completeness, config, marks, readiness, wiring
...
from voci_migrate.audit.wiring import DEFAULT_BUDGET
...
def _suite(
    ...
    *,
    ...
    return Suite(
        ...
        autouse_nodes=len(
            {finding.detail.get("node") for finding in findings if finding.code == "VC008"}
        ),
...
def _async_tests(ground_truth: GroundTruth, defined: dict[str, completeness.Defined]) -> int:
    ...
    voci runs both kinds, so this is not a translation question — it is the denominator for the
