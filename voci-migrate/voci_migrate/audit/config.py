...
`[tool.voci]` rejects unknown keys, so every setting has to be decided rather than carried over,
...
voci spelling blocks nothing on its own, and counting one against every test would swamp the
...
def _ini_findings(ground_truth: GroundTruth, root: Path | None) -> Iterator[Finding]:
    ...
    for key in sorted(written):
        code = matrix.INI_SETTINGS.get(key, "VC309")
