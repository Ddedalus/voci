...
coming, and an autouse fixture, because that is where a `voci.use(...)` line lands.
"""
...
from voci_migrate import matrix, model
from voci_migrate.audit.findings import Finding, Site
from voci_migrate.audit.reach import Reach
from voci_migrate.model import FixtureDef, GroundTruth, Item

# What a conftest override is reported as: specialized, over budget, or undeclarable.
OVERRIDE_CODES = ("VC005", "VC006", "VC027")
...
_SCANNED_ELSEWHERE = frozenset({"VC401"})
...
# value of it voci has. anyio's default is both backends when trio is installed, which doubles
...
def plugin_wired(ground_truth: GroundTruth) -> frozenset[str]:
    ...
    #
    # Narrowed to `anyio_backend` itself rather than every fixture a VC320-coded plugin happens to
    ...
    for fixture in ground_truth.fixture_defs.values():
        ...
        if root in ("_pytest", "pytest"):
            continue
        if matrix.plugin(provider.get(root, root.replace("_", "-"))).code == "VC320":
...
def _backend_findings(ground_truth: GroundTruth) -> Iterator[Finding]:
    """The cases a backend parametrization schedules on a loop voci does not run.
...
def _foreign_fixture(
    ...
    if root in ("_pytest", "pytest"):
        code = matrix.BUILTIN_FIXTURES.get(fixture.argname, "VC220")
    ...
    if recipe.construct.disposition is matrix.MECHANICAL:
        return
    code = "VC322" if recipe.code == "VC322" else "VC030"
    yield Finding(
        code=code,
        message=(
...
def _override_findings(
    ...
    for override in overrides(ground_truth):
        ...
        yield Finding(
            code="VC006" if over_budget else "VC005",
            message=(
...
def _downstream(item: Item, name: str, winner: FixtureDef) -> set[str]:
    ...
    definition of `name` they end up wired to, which in voci is a different object. A fixture
