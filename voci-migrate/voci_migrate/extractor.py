"""The pytest plugin that dumps a suite's collection-time ground truth as JSON:
    ...
    pytest -p voci_migrate.extractor --collect-only -q --extractor-out ground-truth.json
...
This module imports stdlib and pytest and nothing else, including nothing from `voci_migrate`.
...
def pytest_addoption(parser):
    group = parser.getgroup("voci-migrate")
    group.addoption(
        "--extractor-out",
        dest="extractor_out",
        default=None,
        metavar="PATH",
        help=(
            "Write the voci-migrate ground-truth dump here "
...
def pytest_configure(config):
    ...
    if not (MIN_PYTEST <= version < MAX_PYTEST_EXCLUSIVE):
        ...
        raise pytest.UsageError(
            f"voci-migrate's extractor supports pytest {supported}, "
...
def _autouse_by_node(fixturemanager):
    """Autouse fixture names keyed by the node they apply to — where a `voci.use` goes.
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if reporter is not None:
        ...
        reporter.write_line(f"voci-migrate: {note}")
