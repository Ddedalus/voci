...
$ cat .voci-migrate/ground-truth.json
```
...
```console
$ voci-migrate extract tests -o ground-truth.json -- -p no:randomly --ignore tests/slow
```

Where the suite collects somewhere voci-migrate cannot be installed — a container, a locked CI
image — `voci_migrate/extractor.py` is a single file that imports only pytest. Copy it in, load
...
ground_truth = model.load(".voci-migrate/ground-truth.json")
...
- **needs review** — converts, and something about it shifted enough that the converted source
  carries a `VOCI-TODO[category]` marker, or the test behaves differently once tests overlap;
- **blocked** — no conversion path as it stands, because the construct needs a human decision or
  voci provides nothing like it;
...
Every finding cites a code, `VC214` or `VC401`, that names one row of the support matrix: the pytest
...
```console
$ voci-migrate audit --dump ground-truth.json --root ../service --out audit/
$ voci-migrate audit --budget 12     # allow longer generated override chains
```

A conftest fixture that overrides one from a parent directory has no counterpart in voci, where a
...
```console
$ voci-migrate scaffold ../service work/service
voci-migrate/relocation (created at the tip) rebased onto a1b2c3d4e5f6, exported to /home/you/work/service
relocation fixups go in /home/you/.voci-migrate.service.relocation, committed there directly
```
...
The first call creates `voci-migrate/relocation` empty at the suite's own tip; every call after
...
`convert --write` snapshots the export — everything but its own `.voci-migrate/` state — into
`.voci-migrate/baseline/tree`, and records the suite's pytest outcomes into
`.voci-migrate/baseline/pytest-outcomes.json`, before it overwrites anything. That is a safety
...
voci runs at `--concurrency 1` unless `-c` says otherwise. A serial run is what separates "the
...
run under voci: `mechanical_showcase` for the constructs that translate one for one,
