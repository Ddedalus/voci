...
## `just refactor rewire object old.module new.module`
...
Promoting `_x.py` to `_pkg/x.py` with nothing split is `git mv`, not `just refactor move` —
`move` exists to split objects out of a multi-object file, and running it on an already-cohesive
file re-emits it through ruff for no reason and loses git's rename tracking. Follow the `git mv`
with `rewire`/`rewire-module` for every name or whole-module import any other file uses.
