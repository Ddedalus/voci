...
**What it takes is an already-analyzed object, not a raw callable.** FastAPI accepts any callable and introspects it later, at route registration, with `inspect.signature` +
   `get_type_hints`. `@voci.fixture()` has already done that work at decoration time: a `Fixture`
   arrives carrying its scope, its exclusivity token, and its own resolved dependency plan.

### What `@voci.fixture()` accepts
...
The `@voci.fixture` decorator unwraps the generator/awaitable and returns a `Fixture[T]`;
...
async def api_client(
    ...
    async with voci.fastapi.client(
        app,
...
`voci.fastapi.client` replaces `app.dependency_overrides` (and, when `state=` is passed, `app.state`)
...
```python
@voci.parametrize("clock", [FrozenClock(...), RealClock()])
...
```python
@voci.skip("not implemented yet")
@voci.skipif(sys.platform == "win32", reason="posix only")
@voci.xfail("flaky upstream", strict=True, raises=TimeoutError)
@voci.tag("slow", "integration")          # selected with -m
@voci.timeout(30)                          # seconds; overrides --timeout
@voci.solo                                 # runs alone, whole suite drained
@voci.isolated                             # runs in a subprocess on a fresh loop
@voci.parametrize("n,expected", [(1, 2), (2, 4)], ids=...)
```

### Parametrize

```python
@voci.parametrize("payload", [{"a": 1}, {"a": 2}], ids=["one", "two"])
@voci.parametrize("verb", ["GET", "POST"])
...
| Fixture | Notes |
|---|---|
| `voci.tmp_path` | `Path`, unique per test by construction: `basetemp/<sanitized-test-id>` (no scan-and-retry; that is a serial-era artifact — R§7). |
| `voci.tmp_path_factory` | Session-scoped; same numbered-root + retention policy as pytest. |
| `voci.capture` | Access to the current test's captured stdout/stderr text ([09](09-capture-and-logging.md)). |
| `voci.log_records` | The `caplog` equivalent: structured `LogRecord`s captured for this test, plus a `set_level()` context manager. |
| `voci.test_info` | Test id, tags, timeout, worker slot — the `request` replacement, deliberately tiny and read-only. |
| `voci.fastapi.lifespan` | Session-scoped; runs one app's lifespan exactly once per run, since the test client never triggers startup. Writes land in the app's base state ([08](08-patching-and-isolation.md) §3.1). |
| `voci.monkeypatch` | *Not provided.* Use `unittest.mock` (which voci schedules solo) or a DI override — see [08](08-patching-and-isolation.md). |

## 7. Class-grouped tests

Supported as pure namespacing: a class whose name starts with `Test`, with no `__init__`, whose
`test_*` methods are collected with `self` ignored (voci instantiates once per test, no state shared through the instance). No `setup_method`/`teardown_method` — that is what a function-scoped fixture is. Class scope for fixtures is not in the MVP (Q2).
...
```python
with voci.raises(ValueError, match="bad input"):
    ...
voci.approx(0.3)
```
...
- `@voci.fixture(scope=, exclusive=, name=)`, `Depends()`, sync/async, generator teardown.
- `skip`, `skipif`, `tag`, `timeout`, `parametrize`, `solo`.
- `raises`, `approx`, plain asserts.
- Class grouping as namespacing.
- `voci.fastapi.client(app, overrides=, state=, base_url=)` and `voci.fastapi.lifespan(app)` —
...
- **`Fixture.with_()` — derive a fixture with one named dependency replaced.** Prototyped and then
  ...
  `voci.Depends` it has no qualified name to whitelist via `extend-immutable-calls` (it is a
  ...
  Today's replacement idiom — used throughout the examples — is a separate `@voci.fixture()`
...
- `@voci.isolated` (needs the subprocess tier, [08](08-patching-and-isolation.md)).
...
- Parametrized fixtures (`@voci.fixture(params=[...])`) with per-param scope instances — the
  cache-key machinery in [04](04-dependency-injection.md) is already designed for it.
- `class` scope.
- `voci.approx` for nested structures; richer `raises` (`ExceptionGroup` matching, `.group_contains`).
