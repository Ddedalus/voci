"""Tests for voci's `raises`/`approx` assertion helpers."""
...
def test_approx_abs_zero_is_exact_for_complex() -> None:
    ...
    assert complex(1.0) == voci.approx(complex(1.0), abs=0, rel=0)
    assert complex(1.0000000000001) != voci.approx(complex(1.0), abs=0, rel=0)
...
def test_approx_abs_alone_is_not_widened_by_the_relative_default() -> None:
    ...
    assert 1.0000000000001 != voci.approx(1.0, abs=0)  # noqa: SIM300
    assert 1.0000001 != voci.approx(1.0, abs=1e-13)  # noqa: SIM300
    assert 1.00000000000001 == voci.approx(1.0, abs=1e-13)  # noqa: SIM300
    # Same rule on the complex branch.
    assert complex(1.0000000000001) != voci.approx(complex(1.0), abs=0)
...
def test_approx_abs_only_matches_pytest() -> None:
    ...
    for actual, expected, tol in [(1.0000000000001, 1.0, 0), (1.0000001, 1.0, 1e-13)]:
        assert (actual == voci.approx(expected, abs=tol)) == (
...
def test_approx_rel_alone_keeps_the_absolute_floor() -> None:
    ...
    assert 0.0 == voci.approx(0.0, rel=1e-6)  # noqa: SIM300
    assert 1e-13 == voci.approx(0.0, rel=1e-6)  # noqa: SIM300


def test_approx_honors_rel_for_complex() -> None:
    """`rel=` must be honored for complex operands, not just real ones."""
    assert complex(1000, 0) == voci.approx(complex(1000.0001, 0), rel=1e-3)
    assert complex(1000, 0) != voci.approx(complex(1000.0001, 0), rel=1e-12)


def test_approx_repr_reflects_an_explicit_abs_zero() -> None:
    assert repr(voci.approx(1.0, abs=0)) == "approx(1.0 rel=None, abs=0)"


def test_approx_repr_is_built_from_the_default_constants() -> None:
    assert repr(voci.approx(0.3)) == f"approx(0.3 ±{voci.Approx.DEFAULT_REL!r})"


def test_approx_compares_a_list_elementwise() -> None:
    assert [0.1 + 0.2, 0.2 + 0.4] == voci.approx([0.3, 0.6])  # noqa: SIM300
    assert (0.1 + 0.2, 0.2 + 0.4) == voci.approx((0.3, 0.6))  # noqa: SIM300


def test_approx_over_a_list_fails_on_mismatched_length() -> None:
    assert voci.approx([1.0, 2.0]) != [1.0, 2.0, 3.0]
    assert voci.approx([1.0, 2.0]) != [1.0]


def test_approx_over_a_list_fails_on_a_value_outside_tolerance() -> None:
    assert voci.approx([1.0, 2.0]) != [1.0, 2.1]


def test_approx_compares_a_dict_elementwise() -> None:
    assert {"a": 0.1 + 0.2, "b": 0.2 + 0.4} == voci.approx({"a": 0.3, "b": 0.6})  # noqa: SIM300


def test_approx_over_a_dict_fails_on_mismatched_keys() -> None:
    assert voci.approx({"a": 0.3, "b": 0.6}) != {"a": 0.3, "c": 0.6}
    assert voci.approx({"a": 0.3, "b": 0.6}) != {"a": 0.3}


def test_approx_over_a_nested_list_raises() -> None:
    with pytest.raises(TypeError, match="nested"):
        voci.approx([1.0, [2.0, 3.0]])


def test_approx_over_a_nested_dict_raises() -> None:
    with pytest.raises(TypeError, match="nested"):
        voci.approx({"a": 1.0, "b": {"c": 2.0}})


def test_approx_over_a_set_raises() -> None:
    """Sets are unordered, so there is no position to compare by."""
    with pytest.raises(TypeError):
        voci.approx({1.0, 2.0})  # type: ignore[arg-type]


def test_approx_over_a_list_nested_in_a_dict_raises() -> None:
    with pytest.raises(TypeError, match="nested"):
        voci.approx({"a": [1.0, 2.0]})
...
def test_approx_over_a_dict_nested_in_a_list_raises() -> None:
    with pytest.raises(TypeError, match="nested"):
        voci.approx([1.0, {"a": 2.0}])


def test_approx_over_a_collection_honors_rel_abs_and_nan_ok_per_element() -> None:
    assert [1.0, float("nan")] == voci.approx([1.0, float("nan")], nan_ok=True)
    assert voci.approx([1.0, float("nan")]) != [1.0, float("nan")]
    assert [1000.0] == voci.approx([1000.0001], rel=1e-3)  # noqa: SIM300
    assert voci.approx([1000.0001], rel=1e-12) != [1000.0]
    assert {"a": 1.0000000000001} == voci.approx({"a": 1.0}, abs=1e-13)  # noqa: SIM300
    assert voci.approx({"a": 1.0}, abs=1e-13) != {"a": 1.0000001}


def test_approx_over_a_collection_is_symmetric() -> None:
    assert [0.3] == voci.approx([0.1 + 0.2])  # noqa: SIM300
    assert voci.approx([0.1 + 0.2]) == [0.3]
    assert {"a": 0.3} == voci.approx({"a": 0.1 + 0.2})  # noqa: SIM300
    assert voci.approx({"a": 0.1 + 0.2}) == {"a": 0.3}
...
def test_exception_info_value_raises_loudly_before_the_block_completes() -> None:
    ...
    with voci.raises(ValueError) as caught:
...
def test_raises_rejects_asyncio_cancelled_error() -> None:
    with pytest.raises(TypeError, match="CancelledError"):
        voci.raises(asyncio.CancelledError)
    with pytest.raises(TypeError, match="CancelledError"):
        voci.raises((ValueError, asyncio.CancelledError))
...
def test_raises_still_catches_system_exit_and_keyboard_interrupt() -> None:
    ...
    with voci.raises(SystemExit):
        raise SystemExit(1)
    with voci.raises(KeyboardInterrupt):
...
def test_raises_callable_form_returns_a_populated_exception_info() -> None:
    ...
    info = voci.raises(ValueError, boom)
...
def test_raises_callable_form_forwards_args_and_kwargs() -> None:
    ...
    info = voci.raises(ValueError, f, 1, 2, c=3)
...
def test_raises_callable_form_rejects_a_non_callable_func() -> None:
    with pytest.raises(TypeError, match="not callable"):
        voci.raises(ValueError, "not a function")  # type: ignore[call-overload]
...
def test_raises_callable_form_rejects_asyncio_cancelled_error() -> None:
    ...
    with pytest.raises(TypeError, match="CancelledError"):
        voci.raises(asyncio.CancelledError, f)
...
def test_raises_callable_form_match_checks_the_exception_not_the_call() -> None:
    ...
    info = voci.raises(ValueError, f, match="must be 0 or None")
    ...
    with pytest.raises(AssertionError, match="does not match"):
        voci.raises(ValueError, g, match="must be 0 or None")
...
def test_raises_bare_keyword_args_without_func_are_rejected() -> None:
    ...
    with pytest.raises(TypeError, match="without a callable"):
        voci.raises(ValueError, unexpected=1)  # type: ignore[call-overload]
