...
def voci_basetemp_root(tmp_path_factory: pytest.TempPathFactory) -> Iterator[None]:
    ...
    voci on it — including a second run of this suite, whose hundreds of runs would
    sweep this one's live session roots as ordinary leftovers.
    """
    parent = tmp_path_factory.mktemp("voci-basetemp")
