...
freely and in any order — the only exception is `@voci.parametrize`, covered on its own page —
and read the same whether the collector sees them or you do.
...
`@voci.skipif` stacks — a test with several is skipped if any one condition holds — but
`@voci.skip` is scalar: applying it twice to the same test raises `TypeError` rather than picking
a reason to keep silently.

## Expected failures

`@voci.xfail` records a failure as expected instead of skipping the test outright — it still
runs, and a call phase that raises reports `XFAILED` rather than `FAILED`:
...
that count as a failure instead, for an expectation you want removed the moment it stops holding.
...
Every mark but `@voci.parametrize` folds into the same per-function record regardless of order,
so `@voci.tag("slow")` above or below `@voci.skip(...)` means the same thing either way.
`@voci.skip`, `@voci.xfail` and `@voci.timeout` are the three that carry one fixed value rather
than a set — applying any of them twice to the same test has no sensible "last one wins" answer,
so it raises `TypeError` naming the function and the value already set, instead of overwriting the
first one silently.
