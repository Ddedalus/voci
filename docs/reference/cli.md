...
failed, is new, or can't be vouched for always runs. `--affected=verify` runs everything and
reports what the plain flag would have skipped, flagging any disagreement with the outcome that
selection predicted. `--watch` selects the same way on every iteration, on top of re-running
failures. [Running only what changed](../guide/affected.md) covers the summary line, branch
switching, and the opt-in `trace_threads`/`trace_subprocesses` keys for tests that use threads or
subprocesses.
