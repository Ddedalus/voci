...
`--affected` re-runs only the tests whose recorded dependencies — the functions they called, the
files they read, the environment variables they read — no longer match the tree; a test that
failed, is new, or can't be vouched for always runs. `--affected=verify` runs everything
regardless, checks each prediction against the outcome the test actually produced, and reports
any mismatch by name. `--watch` selects the same way on every iteration, on top of re-running
failures. [Running only what changed](../guide/affected.md) covers the summary line, branch
switching, `verify`'s mismatch report, and the opt-in `trace_threads`/`trace_subprocesses` keys
for tests that use threads or subprocesses.
