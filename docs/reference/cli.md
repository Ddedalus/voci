...
Run `voci --affected` to re-run only the tests whose recorded dependencies — the functions they
called, the files they read, the environment variables they read — no longer match the tree; a
test that failed, is new, or can't be vouched for always runs. `--affected-verify` runs
everything regardless, checks each prediction against the outcome the test actually produced,
and reports any mismatch by name. `--watch` selects the same way `--affected` does on every
iteration, on top of re-running failures. [Running only what changed](../guide/affected.md) covers the summary
line, branch switching, `--affected-verify`'s mismatch report, and the opt-in
`affected_trace_threads`/`affected_trace_subprocesses` keys for tests that use threads or
subprocesses.
...
else to the rootdir when \[tool.voci\] fixed it and the current directory otherwise.
...
Run only tests whose @voci.tag(...) names satisfy this boolean expression, e.g. 'slow and not
...
Default: 16, or \[tool.voci\] concurrency if set.

### `--timeout SECONDS`

Per-test setup+call budget, in seconds, overridable per test with @voci.timeout(...). A test that
exceeds its budget is reported as TIMEOUT rather than FAILED/ERROR. Must be positive and finite.
Default: no limit, or \[tool.voci\] timeout if set.
...
\[tool.voci\] loop\_watchdog if set.
...
earlier one, and all of them outrank \[tool.voci\] filterwarnings. Every warning a run raises is
