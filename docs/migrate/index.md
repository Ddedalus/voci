...
`voci-migrate` takes a pytest suite and turns it into a voci one. It is a separate distribution
from voci: the runtime never depends on it, and nothing it installs ends up in the migrated
suite.

```bash
uv pip install voci-migrate    # or: pip install voci-migrate
```
...
| Command | Does |
|---|---|
| `voci-migrate extract` | asks pytest what it resolved for the suite, and writes it down |
| `voci-migrate audit` | classifies every construct and reports what migrating would cost |
| `voci-migrate convert` | rewrites the suite in voci's spelling |
| `voci-migrate verify` | runs both runners and compares them test for test |
...
directory builtins, `raises` and `approx` all have a voci counterpart the conversion writes for
you:
...
| `autouse=True`, `@pytest.mark.usefixtures` | one `voci.use(...)` on the module or package |
| `@pytest.mark.parametrize`, `pytest.param` | `@voci.parametrize`, `voci.case`, with pytest's ids kept verbatim |
| indirect parametrization | `params=` on the fixture itself |
| `skip`, `skipif`, `xfail`, custom marks | `@voci.skip`, `@voci.skipif`, `@voci.xfail`, `@voci.tag` |
| `capsys`, `caplog`, `tmp_path`, `tmpdir` | `voci.capture`, `voci.log_records`, `tmp_path`, `voci.tmpdir` |
| `pytest.raises`, `pytest.approx` | `voci.raises`, `voci.approx` |
| `pytest-asyncio`, `anyio`, `event_loop` fixtures | deleted — voci runs async tests itself |
| `pytest-xdist` | deleted — voci is concurrent within one process |

Some of it has no counterpart, and a suite leaning on one of these has to give it up or keep that
part under pytest:

| pytest | Why voci has nothing for it |
| --- | --- |
| `unittest.TestCase`, doctests, nose-style collection | voci collects functions and `class Test*` methods, and nothing else |
...
| `pytest.warns`, `recwarn`, `deprecated_call`, `filterwarnings` | the warnings filter is one process-global list, and voci's tests share the process |
| `capfd`, `capsysbinary`, `capfdbinary` | voci captures by replacing `sys.stdout` and `sys.stderr`, so a write to file descriptor 1 by a subprocess or a C extension goes uncaptured, and there is no binary variant |
| `pytestconfig`, `cache`, `record_property`, `pytester` | each is a handle on pytest's own machinery |
| `pytest.importorskip`, `pytest.xfail()` as a statement | a decision made partway through a body has no runtime call to make it with — guard the import and use `@voci.skipif`, or mark the test `@voci.xfail` outright |
| a non-asyncio event loop — trio, tornado | voci runs the suite on one asyncio loop |
...
the whole process shares — makes voci schedule that test alone, since nothing else can safely run
...
```console
$ voci-migrate extract tests
42 tests collected in 0.03s
voci-migrate: wrote ground truth for 42 tests to .voci-migrate/ground-truth.json
```
...
```console
$ voci-migrate extract tests -- -p no:randomly --ignore tests/slow
```
...
Where the suite collects somewhere `voci-migrate` cannot be installed — a container, a locked CI
image — `voci_migrate/extractor.py` is a single file that imports only pytest. Copy it next to
...
VC003  marker      2  fixture scope="class" or scope="package"
VC005  mechanical  1  a fixture overriding one visible from further out
VC103  marker      1  @pytest.mark.skipif with a string condition
VC301  mechanical  1  testpaths
...

types: 6 of 14 fixtures state no return type, costing 11 injected parameters — the report lists them worst first

wrote .voci-migrate/migration-report.md and .voci-migrate/findings.json
```
...
Every finding cites a code — `VC003`, `VC401` — that names one row of the support matrix: the
...
lands that way. The exception is a fixture voci itself provides — `tmp_path`, `capsys` and the
rest — whose type comes from voci's own declaration, since your suite never said what they
return either way.
...
translated:
  VC104  6  @pytest.mark.skip, @pytest.mark.skipif(bool)
  VC101  4  @pytest.mark.parametrize
  ...
...
```python
# VOCI-TODO[VC003]: Confirm the fixture tolerates being shared more widely, or split it.
```

Grepping for `VOCI-TODO` is the review queue the conversion leaves behind.

## Verify against pytest

`verify` runs both runners and compares them test for test: pytest on the tree that still holds
the pytest suite, voci on the converted one. Ids survive the conversion verbatim, so the two
verdicts line up by id, and every id they disagree about is the review queue.

```console
$ voci-migrate verify --before ../service-pytest --after .
1188 tests under pytest   1188 under voci   1183 agreed   5 diverged
outcome (5) — ran under both and ended differently
  tests/test_registry.py::test_serializer_must_have_meta            passed -> failed
  ...

wrote .voci-migrate/verify-report.md and .voci-migrate/verify.json
```
...
```console
$ voci-migrate verify --record --before .   # -> .voci-migrate/pytest-outcomes.json
$ voci-migrate convert --write
$ voci-migrate verify --after .
```

Both halves run in one environment, since one command runs both. Where pytest's half has to run
somewhere `voci-migrate` cannot be installed, `voci_migrate/outcomes.py` is a single file that
...
voci runs at `--concurrency 1` under `verify` unless `-c` says otherwise. A serial run is what
...
```console
$ voci-migrate verify --after . -c 16
```
