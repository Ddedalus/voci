...
## 0 — De-autouse (`VX027` only)

`VX027` fires when any of winner, `[-2]` or `D` is `autouse=True`. Drop `autouse` from every
autouse definition in the chain and name the fixture where it applied. Module-level
`pytestmark` converts as `VX009` (mechanical); a mark on individual tests converts as `VX010`
...
Re-audit: the finding becomes `VX005` or `VX006`. Continue with strategies 1–5.
