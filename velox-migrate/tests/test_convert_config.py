"""Tests for velox_migrate.convert.config: what a suite's pytest settings become under velox.
...
write the settings velox does have a key for are built on top of those dumps, since neither corpus
suite writes one.
"""
...
def test_the_settings_velox_has_a_key_for_are_carried(
    ...
    # velox's own key order, so the same suite always writes the same table.
...
def test_the_rendered_table_is_toml(fixtures: GroundTruth, tmp_path: Path) -> None:
    # Hand-rendered, since velox-migrate depends on libcst alone and `tomllib` only reads.
    ground_truth = wrote(
        fixtures,
        tmp_path,
        testpaths="['tests']",
    ...
    assert tomllib.loads(translation.table)["tool"]["velox"] == {
        "testpaths": ["tests"],
        "timeout": 30,
        "test_file_patterns": ["check_*.py"],
        "ignore": ["build"],
...
def test_a_value_that_cannot_be_rendered_is_dropped_rather_than_guessed_at(
    ...
    ground_truth = wrote(
        ...
        python_files="[]",  # written with no value; an empty velox list would collect nothing
        norecursedirs="'build'",  # a bare string where velox takes a list
        timeout="'soon'",
    )
...
def test_a_quote_or_a_backslash_in_a_value_is_escaped(
    ...
    assert tomllib.loads(translation.table)["tool"]["velox"]["ignore"] == [
        'say "no"',
        "build\\out",
    ]
...
def test_a_sub_table_counts_as_a_configured_velox(hazards: GroundTruth, tmp_path: Path) -> None:
    (tmp_path / "pyproject.toml").write_text('[tool.velox.env]\nTZ = "UTC"\n', encoding="utf-8")
