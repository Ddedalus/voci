...
RELOCATION_BRANCH = "velox-migrate/relocation"

#: Where `convert --write` snapshots `dest`, relative to `dest` itself.
BASELINE_DIR = Path(".velox-migrate") / "baseline"
TREE_DIR = "tree"
...
def snapshot_baseline(root: Path) -> Path:
    """Copy `root`'s current tree, minus this tool's own state, into `root/.velox-migrate/
    ...
    tool_state = (root / ".velox-migrate").resolve()
...
def _export(scratch: Path, dest: Path, *, source: Path, branch: str) -> None:
    marker = dest / ".velox-migrate" / _MARKER_NAME
    if dest.exists():
