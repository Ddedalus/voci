...
One rule per support-matrix row: it matches a single pytest source form, writes the velox spelling
...
to all of them. And a rule writes no import: it emits `velox.`-qualified names, assuming
`import velox`, and any other module a rewrite needs travels in the `Applied` record for the
caller to act on.
"""
...
def velox(name: str) -> cst.Attribute:
    """`velox.<name>`, the spelling every rule emits."""
    return cst.Attribute(value=cst.Name("velox"), attr=cst.Name(name))
