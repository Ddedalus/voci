...
this fixture without naming it — and velox says it once, as `velox.use(...)` on the module or the
...
Two things follow from velox reading a package declaration by walking up from the test file. The
...
its own body has to say so after that body — a `velox.use` above the `def` it names would read a
...
def container_for(node: str) -> str | None:
    ...
    A class's node has no counterpart: `velox.use(...)` covers a module or a package, and a fixture
...
def packages(
    ...
    *,
    ...
    velox reads a package's declarations by walking up from the test file and stopping at the first
...
def apply(module: cst.Module, references: Sequence[str]) -> cst.Module:
    """`module` with a `velox.use(...)` naming every one of `references` it does not name yet.
...
def _declared(module: cst.Module) -> frozenset[str]:
    """Every name a `velox.use(...)` at the top of `module` already declares."""
    found: set[str] = set()
...
def _is_use(expression: cst.BaseExpression) -> bool:
    """Whether `expression` is a `velox.use(...)` call, spelled as this module writes one."""
    match expression:
        case cst.Call(
            func=cst.Attribute(value=cst.Name(value="velox"), attr=cst.Name(value="use"))
        ):
            return True
        case _:
            return False
...
def _statement(
    ...
    """`velox.use(...)`, spaced from what it follows the way a formatter would space it."""
    blanks = 2 if isinstance(after, cst.FunctionDef | cst.ClassDef) else 1
    return cst.SimpleStatementLine(
        body=[
            cst.Expr(
                cst.Call(
                    func=cst.Attribute(value=cst.Name(VELOX), attr=cst.Name(USE)),
                    args=[
