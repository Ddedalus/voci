...
# The type each velox built-in hands the parameter it is injected into, and the import that names
# it. Written down rather than inferred, because the suite's own sources say nothing about what
# `tmp_path` returns and velox is not a dependency of this tool. Most are spelled through the
# `import velox` a converted module already has, so they need no import of their own;
# `test_a_written_builtin_type_matches_what_velox_declares` pins every row against velox itself,
...
BUILTIN_TYPES: Mapping[str, tuple[str, TypeImport | None]] = {
    ...
    "tmp_path_factory": ("velox.TmpPathFactory", None),
    "tmpdir": ("velox.LegacyPath", None),
    "tmpdir_factory": ("velox.LegacyTmpPathFactory", None),
    "capsys": ("velox.Capture", None),
...
class Resolver:
    ...
    def builtin(self, argname: str, consumer: str) -> Typed | None:
        """The annotation an injection of pytest's `argname` gets from velox's counterpart.

        No `Degraded` row when there is none: a built-in this does not name is one velox has no
        ...
        spelling = self._decide(origin.bound, hint="velox", consumer=consumer, origin=origin)
        return Typed(
