...
(`teardowns`). `report` does both at once, and is written above a directory that redefines what it
depends on, so the subtree gets a copy of it that has to be rewritten the same way.
"""
...
@pytest.fixture
def report(request, teardowns, finished):
    request.addfinalizer(lambda: finished.append("report"))
    return {"dsn": request.getfixturevalue("settings")["dsn"], "seen": list(teardowns)}
